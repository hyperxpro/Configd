...
/**
 * The single transport boundary out of a {@link FanOutSessionCore}. The session emits
 * {@link EdgeFrame}s through this sink and never touches a socket, TLS, or
 * {@code java.net} type directly - the live server and the simulator each provide an
 * implementation.
 *
 ...
 * {@code <= queueFrames}), so {@code offer} returning {@code false} is an additional,
 * lower backpressure signal - a would-block at the transport - that the session also
 * treats as a demotion trigger (never an unbounded buffer, never a silent drop).
 */
