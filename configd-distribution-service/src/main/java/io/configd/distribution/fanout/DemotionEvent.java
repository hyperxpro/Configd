...
/**
 * The structured event a {@link FanOutSessionCore} records on every streaming→catch-up
 * demotion (C1 design §4: "never a silent drop; every demotion emits a structured log
 * event + metric with (session, cursor, lastAckedSeq, reason)"; CT-26). C4 layers
 * disconnect / quarantine policy on top of these events.
 *
 * @param cursor       the session cursor at the moment of demotion (the highest seq the
 *                     server had streamed) — the cursor evidence CT-26 requires
