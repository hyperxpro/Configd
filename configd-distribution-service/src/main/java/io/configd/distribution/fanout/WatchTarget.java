...
/**
 * The target of a watch (RFC §2 W2-2): the {@code (scope, kind, path)} a watch subscribes
 * to, plus the {@code full_chain_verify} delivery-mode flag (W8-4). A small, dependency-free
 * value type — it deliberately carries the scope as a raw {@code int} (not a
 * {@code ConfigScope}) so the fan-out plane stays decoupled from the config-store / api
 * scope enum and so it can be passed verbatim to the {@link WatchAuthorizer} SPI.
 *
 * <p>This type couples the authorization surface and the per-watch routing surface but keeps
 * them <b>distinct</b> (RFC §2 §7 vs §5):
 * <ul>
 *   <li><b>Authorization</b> (the gate) consumes the whole target via {@link WatchAuthorizer}
 *       — authorize <em>all</em> of it or reject (W7-2). The {@code full_chain_verify}/FULL
 *       flag routes the adapter to the root effective target (W7-3).</li>
 ...
 *       keys precisely because it was gated by a root-scope grant — never narrowed-as-authz,
 *       the forbidden false-completeness bypass, W7-2).</li>
 * </ul>
 *
 * @param scope           the {@code ConfigScope} ordinal as a raw {@code int} (0=GLOBAL,
 *                        1=REGIONAL, 2=LOCAL; §1 A2-1)
 * @param targetKind      {@link EdgeFrame#WATCH_TARGET_KEY} / {@link EdgeFrame#WATCH_TARGET_PREFIX}
 *                        / {@link EdgeFrame#WATCH_TARGET_FULL}
 * @param path            the canonical §1 path (empty for FULL); the literal match prefix/key
 * @param fullChainVerify the untrusted-edge verbatim mode flag (W8-4); requires root scope
 *                        (W7-3) and, in v1, is served as a FULL key stream (W8-4 / brief §6b)
 */
public record WatchTarget(int scope, int targetKind, String path, boolean fullChainVerify) {
    ...
    public WatchTarget {
        ...
        // §1 path grammar are validated upstream (WatchTargetValidator) and surfaced as
    ...
    /**
     * The per-watch routing filter (RFC §2 W5-6) — true iff a mutation on {@code key} belongs
     * to this target. This is <b>routing, not authorization</b> (the gate already authorized
     * the whole target):
     * <ul>
