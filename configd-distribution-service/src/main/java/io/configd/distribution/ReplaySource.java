...
/**
 * The authoritative recovery seam (ADR-0034 §replay contract) a consumer uses
 * after {@link CommitNotificationSource#readSince(long)} returns a GAP.
 *
 ...
 * log+snapshot. Post-RR-003 the durable prefix reconstructs ALL committed state,
 * so any notification the cache evicted is still recoverable here.
 *
 * <h2>What Session 3 gets (the contract)</h2>
 ...
 * {@code readSince(S)} to tail forward. This is exactly-once over <em>effect</em>:
 ...
 * sufficient for the §4.6 contract and far cheaper than reconstructing the
 * full historical delta sequence from the WAL: the edge data plane is
 * <em>eventually-consistent</em> and applies cumulative state, not an audited
 ...
 * the data plane nothing it can observe; it is therefore out of scope for this
 * contract (ADR-0034 §replay). If a future auditing consumer needs the exact
 * historical mutation stream, that is a separate, WAL-backed replay seam.
 */
