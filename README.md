...
- **Linearizability under fault.** A real Jepsen-grade adversarial matrix — `kill -9`+restart, network partitions (single- and multi-node, quorum-breaking), `SIGSTOP`/`SIGCONT` pauses, packet loss, clock skew, and overlapping combinations — runs on 3- and 5-node clusters across encryption / auth / clock-skew / multi-shard postures, with every recorded history checked linearizable by the trusted Porcupine checker on the release commit (`docs/measurement/e1-faulted-linz-2026-07-10/`). Endurance (a ≥72 h soak) is still pending. Edge staleness bounds held with wide margin (p99 of 13 to 24 ms against a 500 ms bound).
