...
> **Rewritten (S3 doc pass, 2026-06-12, per c4-signoff-review C4-B / the RR-050
> runbook-fiction family):** the previous version directed operators at `CatchUpService`
> and `SlowConsumerPolicy` (both DELETED at S3/C4 — superseded shelfware, RR-088), at
...
RR-100). The per-session machinery is self-limiting by design: bounded per-subscriber
queues, paced transfers (RR-102 would-block pause/resume), ack-lag demotion, and the
...
4. Structured logs on the CP: `edge_fanout_consumer_transition` lines carry identity,
   from→to, reason, cursor evidence, and window counts — the storm's shape in one grep.
...
4. **Tune thresholds only by config and only deliberately** (`edge.fanout.policy.*` on
   the CP; each is a named config with a metric — see architecture §7's amended table).
...
- `gates/e2e-compose-scenario.sh` phase 3 is the executable miniature of this runbook
  (partition → ladder → re-bootstrap → convergence), captured green in
  `docs/session-3/captures/e2e-compose-scenario-run.txt`.
...
- Snapshot-vs-replay selection is automatic (`decideMode`; RR-100) — there is no
...
- Size `edge.fanout.maxSessions` (admission bound) and the per-session
  `queueFrames`/`snapshotChunkBytes` for the worst-case fleet reconnect; the RR-102
...
- A long-PAUSED transfer (wedged-but-open transport) has no dedicated stalled-transfer
  signal yet — queue-depth shows pressure only; S6 observability item
  (c5-signoff-review F2).
- Deep-store bootstraps can emit one redundant re-demote envelope per ack RTT
  (deliberate self-healing; S6 efficiency item, c5-signoff-review F3).
- RR-103 (consensus per-peer inflight leak, S4): a wedged CP follower can starve the
  edges subscribed to it — the edges' staleness ladder + multi-endpoint failover is the
  containment; the fix is S4's with RR-095.
