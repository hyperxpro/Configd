...
| Durable-prefix continuity | `durable_prefix_no_gap` | Raft recovery + apply (`RaftNode` ctor `:257`, `applyCommitted` `:1655`) | a committed index below a snapshot boundary has restorable bytes (no silent skip of committed state) | Metric `invariant.violation.durable_prefix_no_gap` + SEVERE log; refuses to advance `lastApplied` past the gap |
| Single-writer apply | `apply_owner_thread` | Config state-machine apply (`ConfigStateMachine.apply` `:280`) | every `apply` runs on the owner thread bound on first apply (single-writer tripwire) | Metric `invariant.violation.apply_owner_thread` (+ `StateMachineMetrics.onApplyOwnerThreadViolation`) + throw in test/sim |
