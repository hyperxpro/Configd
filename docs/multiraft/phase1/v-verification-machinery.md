...
> This is that deliverable. Status: **built, green, proven non-vacuous, committed.**
...
| `ShardRouters` | `configd-testkit` test | A correct hash-within-scope reference router (the V stand-in until C1) + a deliberately non-functional `rotating` router (non-vacuity). |
...
| **Cross-shard isolation** | `faultShardMajority` kills one shard (no quorum); the others must keep committing (`commitsAdvancedOn`) and stay safe | the routing leak that *would* break isolation is the same one caught RED by `nonVacuity_crossShardRedirect` |
...
- **Cross-shard isolation** is partly structural here: independent per-shard harnesses cannot leak a fault
  by construction, so the green test confirms the *machinery* doesn't wrongly couple shards, and the
  non-vacuity comes from the routing-leak RED. **C3 adds the complementary fidelity** — S groups per
  physical node on the Phase-0 owner-executor pool (the shared-node, correlated-node-fault surface), where
  isolation is NOT structural and must be proven on the real `MultiRaftDriver`.
