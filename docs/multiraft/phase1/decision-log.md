...
| **DL-P1-09** | **Scoped the heavy `ConfigdServer` N-group production wiring (C2 routing-wiring + C3 server N-group loop + C4 config/observability) + the wire-format work as the operator-gated EC2-prep**, rather than half-build it this session. The sharding LOGIC (ownership, routing, redirect, the DISCLAIM guard, N=1 equivalence) is fully SIM-verified (10,033/0 at ≥10k seeds); the server wiring is the deployment bridge to the EC2 measurement. | (1) Charter hard rule #9 — never leave routing/sharding half-built; "stop clean beats finish dirty"; the consensus-setup block (`ConfigdServer.java:361-468`) is large consensus-adjacent surgery coupled to N-group registration. (2) Charter §5 — verification is SIMULATOR-based; the production wiring enables the (money-gated) EC2 run. (3) Context budget. Specified precisely at file:line in `handoff.md` §4. | Yes — next session implements handoff §4.A–D |
