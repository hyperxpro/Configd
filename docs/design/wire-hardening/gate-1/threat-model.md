# Hostile-Sender Threat Model (Configd Wire Protocol)
...
The companion `attack-matrix.md` is the per-frame x attack-class status table this
...
- *Def:* peer stamps a version the reader does not implement, hoping for misparse or a
  silent downgrade.
- *Mechanism:* set `ver` byte != known. Raft: any != `0x02`. Edge: any not in {0x01,0x02,0x03},
...
- *Def:* a signed length read as negative, or `count x record` overflowing to a small
...
- *Defense:* hard element caps **and** "declared count can't fit remaining bytes" pre-checks
  ...
  `countxrecordSize <= remaining` (`EdgeFrameCodec:880,914,940`). No recursion in either codec
...
- *Status:* **DEFENDED**, with a hardening note: worst-case aggregate buffer is
  `1024 x 16 MiB ~ 16 GiB` (Raft) / `1024 x 2 MiB ~ 2 GiB` (edge) if every peer parks a
...
| **G-5** | **Low** | 9 | **Large aggregate buffer ceiling.** `cap x max-frame` ~ 16 GiB (Raft) / 2 GiB (edge). Bounded but generous; consider a smaller default frame cap or a global in-flight-bytes ceiling. | `FrameCodec.MAX_FRAME_SIZE`; `EdgeFrameCodec.MAX_EDGE_FRAME_SIZE` |
| - | Info | 11 | CRC32C is not a MAC; all authenticity rests on TLS. Correct by design - documented here so the RFC states it normatively (no wire-level integrity without TLS). | - |
...
Closing these gaps calls for: (1) an edge inbound read-idle deadline mirroring `inboundReadTimeoutMs`
(G-2), (2) a `senderId<->cert` binding decision (G-3), (3) replacing per-frame stack-trace prints
