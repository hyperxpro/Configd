...
Gate 1 deliverable. Grounds the Configd wire-hardening effort in how four mature,
...
about the `(long)` cast. The prior-art reinforcement: make this a **property-test
invariant for Gate 3** — for every length/count field, feed `{-1, Integer.MIN_VALUE, 0,
...
outside a snapshot exchange). Audit for this in Gate 2.
...
`inflate-then-check`. **(b)** Our per-element bound is "count × min-element-size ≤
...
Verify in Gate 2/Gate 4 that a hostile `SNAPSHOT_BEGIN` claiming a huge `chunkCount`/
...
- **QUIC** anti-amplification: a server MUST NOT send more than **3×** the bytes it has
...
1. **A per-connection pre-auth byte cap** distinct from the 16 MiB / 2 MiB frame cap. An
   ...
   peer authenticates.
2. **Amplification discipline** — QUIC's 3× rule. Our transports are TCP+TLS (TCP's
   ...
   is worth an explicit check in Gate 2.
...
needs explicit verification in Gate 2/Gate 4. A partial-frame that never completes must
...
3. **Negative/overflow-safe length math** (`(long)` casts, `< 0` guards, `count × min-elem
...
**Thin vs. prior art (candidate Gate 2 work):**
...
- **QUIC** — RFC 9000: §6 (version negotiation), §8.1 (3× anti-amplification), §14.1
