...
> **Session:** Auth-SPI design, 2026-06-28. **Status:** design + recommendation only — no provider built, no
> wiring, no production code. A compile-checked signature sketch lives in [`sketch/`](sketch/) and is the
> normative reference for the signatures quoted here (it compiles and its design-contract checks pass on
> Corretto JDK 25). Reads on from [`built-reality.md`](built-reality.md) (the N = 2 it generalises),
> [`prior-art.md`](prior-art.md) (Vault/JAAS/servlet/SPIFFE), and the namespace access-control design
...
- It **MUST NOT** decide whether a principal may perform an operation on a path. That is the **in-core**
  ...
  **only authentication is the SPI**. This split is the whole design (the Vault auth-method → core-policy split,
...
([§3](#3-the-credential-abstraction)) is intentionally **not** sealed at the case level for *implementers* —
the authz consequence is that an authenticator claims credentials by `canAttempt` rather than by exhaustive
`switch`.
...
`BearerTokenAuthenticator` ([§7](#7-default-providers-v1--in-core-zero-new-dependency)) unchanged in behavior;
...
- **`CertChain`** — the **already-verified** peer chain (the TLS stack ran `setNeedClientAuth(true)`;
  ...
  URI / SPIFFE ID for a SPIFFE deployment, [`prior-art.md`](prior-art.md) §4.2) — it does **not** re-do path
  validation, which is the platform's job (RA-6).
...
- **`attributes`** — identity claims (OIDC claims, the SAN URI, a tenant id) for **audit** and **future ABAC**.
  Not consulted by v1 authz (which is role-based); a forward slot.
...
## 7. Default providers (v1) — in-core, zero new dependency
...
- **No Configd-minted session/token (v1).** Unlike Vault (which issues a token carrying policies), v1
...
with **no core change beyond the seam** — interface-first, exactly as the charter asks.
