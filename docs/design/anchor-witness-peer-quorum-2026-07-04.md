...
(`-Dconfigd.raft.witnessStrict=true`, default fast-vote, because a full-strict default broke
3-node failover in testing); the witness is **armed only when `tcpTransport != null`** (a
configured multi-node cluster), inert otherwise; a latched node advertises its frozen
`bootAnchorSeq` to avoid a false-refuse on a rolling restart. The N>=5 fast-vote
grant->witnessed residual is documented in the frozen-format doc's threat-model section. This
...
**One honest boundary: the strict dimension splits into boot vs vote.** The witness
has two strict dimensions, decoupled because a single "full strict" default
broke leader failover (a 3-node cluster failed to re-elect after the leader was
...
quorum (self + one peer) - which had a packet-loss-reachable false-pass an earlier draft wrongly called
...
default is **fast vote** (grant immediately; failover preserved). This is strictly better than the
un-witnessed baseline (R-a' wholly un-witnessed) and closes R-a' at N=3 by default.

No frozen-format change is required.
...
The witness has two strict dimensions, and they are decoupled because a
single "full strict" default broke leader failover (a 3-node cluster failed to
re-elect after the leader was killed: in full strict a survivor cannot grant a usable vote until a
peer-majority acks its announce, and the dead node can never ack). Resolution: **the strict boot gate is
...
freeze-alignment result - the frozen interface is realized as-is, with no extension needed.
