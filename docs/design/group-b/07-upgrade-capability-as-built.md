# Upgrade Capability: the `_acl/format` Fix and the Normative Contract

The verdict on Configd's upgrade path: **the door is open.** Every wire and at-rest binary format is
version-discriminated and fails closed on an unknown version, so a mixed-version cluster cannot
silently misparse or corrupt data. This document closes the **one** residual silent-misparse vector
(the `_acl/` text policy format), resolves the one open forward-compat question about the cloud-KMS
encryption context, and records the normative upgrade contract.
...
## 1. The `_acl/format` version sentinel
...
## 2. The keyring needs **no** encryption-context slot

The frozen keyring's cloud-KMS entry persists `(term, wrapAlgId, nonce, wrappedRoot)` with no
encryption-context/AAD slot. The open question was whether the real Vault provider's unwrap context
is derivable, or whether `KEYRING_FORMAT_VERSION` would need a bump before the format is treated as
permanently frozen.

**Conclusion: no keyring slot and no `KEYRING_FORMAT_VERSION` bump are needed.** Verified against the
external-KMS integration:

- The external-KMS seam sits at the **keyring-custody secret** (the IKM the two keyring-wrapping keys
...
- The keyring entries themselves therefore stay **local** (`WRAP_ALG_LOCAL_GCM`), AES-256-GCM-wrapped under
  ...
  the external-KMS custody model never writes it (the cloud custody lives in `raft-kms-root`, not a keyring entry).

So the non-derivable-context case does not arise: the only external context is `nodeId`,
...
**At-rest format inventory.** External-KMS support added one new at-rest
...
## 3. The normative upgrade and format-compatibility contract

> ### C0 - Frozen formats are permanent.
> Every wire and at-rest format in the inventory (§2, plus `raft-kms-root` above)
...
> reject the load", never "guess". (Verified for every format in the inventory in §2, including `raft-kms-root`.)
>
...
> (e.g. `raft-kms-root` took the new `RKMS`, sibling to the family). This prevents a resurrected value
...
- **OQ-4 (apply the `PolicySerializer` fix now?).** **DONE - §1.** The ACL version knob
