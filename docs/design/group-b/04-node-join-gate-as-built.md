# Node-Join Authorization Gate

This records the node-join gate's design and the honest correctness boundary for its two dormant
modes.

**One-line principle (unchanged across etcd / CockroachDB / ZooKeeper):** *interior (Raft peer) membership
requires a marker a mere client credential can never present.* Configd already implements the reserved-
...
## mTLS node-join strengthening

All four changes compose with the existing Layer-1 (handshake -> resolve -> pin) and Layer-2 (`senderId` ==
pinned; in-body `leaderId`/`candidateId` == sender) enforcement from the wire-hardening work - they are the
...
4. **Config via `ConfigSource`.** `PeerIdentityPolicy.fromConfig(ConfigSource)` replaces `fromSystemProperties()`
   (retained as a byte-identical delegate). All node-join keys flow through the config SPI (system
...
|---|---|---|---|
| **mTLS** | **live** | marker RDN value (default `CN`) or SAN URI  in  `allowedNodes` -> a specific `NodeId`; optionally must also chain to the separate peer CA (R3) | marker absent / unparseable / not in allow-list -> `null` -> dropped at Layer 1 |
...
| T11 | Netty <-> JDK accept/reject + metric parity | both binding tests run the same scenarios; `*RaftTransportContractTest` x3 tiers |

**Byte-identity.** No node-join config + auth-off => every path is the prior, pre-hardening path: `fromConfig` on an unset
...
(`*RaftTransportContractTest` x3, `RaftTransportMtlsAttackTest`, `TlsManagerTest`, `ConfigdServerTest`).
