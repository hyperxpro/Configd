...
The files under `docs/prr/` (production-readiness-review reports: `performance-report.md`,
`verdict.md`, `inventory.md`, etc.) are **point-in-time snapshots** retained as historical record.
They are **not** the authoritative current state and **must not be cited as current evidence**. They
are intentionally **not rewritten** — rewriting dated records would itself be dishonest.
