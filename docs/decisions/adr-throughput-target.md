# ADR (multi-Raft): Correct the §0.1 Write-Throughput Target to a Derived, Falsifiable Aggregate (Per-Shard Rate × Shards × Efficiency)
...
§0.1 (`PROMPT.md:18`) sets **Global write QPS: 10k/s baseline, 100k/s burst.** This number was
...
- **S7.5 — the first real-hardware measurement** (m6id.4xlarge, 16 vCPU, instance-store NVMe, 3
  co-located nodes; `throughput-part2.md`, RR-113) — found the as-built single group sustains
...
S7.5 root cause (single-thread heartbeat starvation) is an *implementation* ceiling, not an
architectural one — so with the consensus-thread fix (coalesced heartbeats + sharded tick-executor
pool + per-tick broadcast-coalescing; `configd-analysis.md` §3) plus group commit/batching, the
...
**Re-express the §0.1 write-throughput target as a derived, falsifiable aggregate:**

```
sustained_aggregate_commit_rate  =  per_shard_knee  ×  N_shards  ×  efficiency_factor
```

- **`per_shard_knee`** — the measured stable single-group commit rate on the *deployment* hardware,
  after the consensus-thread fix + group commit. Floor today: **~800/s** (S7.5, co-located). To be
...
**The corrected §0.1 sustained target stays 10k/s, but as a DERIVED aggregate, not a single-group
...
HB). So `800 × 16 × 0.75 ≈ 9.6k/s` is **not "≈ 10k/s validated"** — it is the *balanced, all-shards-live
lab ceiling* of a measurement plan. Carry the realistic discounts into the headline as a **range**:
balanced/all-live ≈ `N×knee×eff` (≈9.6k/s at the as-built knee); under a node outage ≈ `(2/3)N×knee×eff`
...
met iff the measured `per_shard_knee × N × efficiency` on real hardware meets it, and the path to move
...
as `429 + Retry-After`. S7.5 validated this at the single-group scale (100k/s offered → 565/s
committed, 680k× `429`, only 1,297× `503`, 5 elections — no churn-collapse; admission control =16). The
...
- **Honesty over aspiration.** The old 10k/s was modeled, attached to a topology that measurement
  ...
  *can* be validated or falsified on hardware — which is what stops the build arc from chasing a number
  nobody validated (the §6 charter directive).
- **Keeps the §0.1 contract intact in spirit** (ADR-0031 precedent: grow the design to meet the
...
- **Separates the two regimes the old number conflated:** *sustained durable commit* (bounded by
  per-shard consensus × N) vs *burst absorption* (bounded by admission-control shed behavior). They
...
- **etcd single-group-with-batching ~10k/s** as the per-shard envelope ceiling (and etcd's stance that
  multi-raft is an application-layer concern — `prior-art.md` §4, L5).
- **CockroachDB / TiKV** "throughput scales with the number of (well-scheduled) groups" — the aggregate
  = N × per-group, with liveness cost held flat by coalesced heartbeats (`prior-art.md` L1, L5).
...
- **Lower the target to the measured ~800/s.** Rejected: defeats the purpose of adopting multi-Raft and
  silently downgrades the §0.1 contract (the failure mode ADR-0031 exists to prevent).
...
- The build arc's success metric becomes **measurable**: prove `per_shard_knee × N × efficiency ≥
  10k/s` on dedicated multi-box hardware, with HdrHistogram p50/p99/p999 and the disk/CPU evidence
  rails S7.5 established.
- **A dedicated-host single-group re-measurement is owed** before N is frozen (co-location confound,
  RR-113 / `throughput-part2.md` §D). N stays a deploy-time constant so a higher knee can lower N
  without a reshard.
- RR-113 (P0) is the tracking row; this ADR defines what "resolved" means for it (the aggregate target
  ...
  resolution.
- §0.3 "surpass-Quicksilver" scorecards that cite write throughput must use the derived aggregate with
...
- **"N × per-shard assumes uniform load; a hot shard means the aggregate is a fiction."** *Cross-links
...
Reuse the S7.5 rails (CO-corrected open-loop driver, HdrHistogram, per-phase iostat/mpstat/pidstat,
`elections_total` per rate) at N shards on dedicated hosts: a rate ladder per shard count proving the
per-shard knee and the aggregate; a 100k/s burst proving graceful shed at N; the amplification guard
(`configd-analysis.md` §7) proving heartbeat-emit latency stays under the election floor as N scales.

## Related

ADR-0031 (precedent: renegotiate a §0.1 target via ADR), ADR-0023 / RR-047 (the modeled, unmeasured
10k/s), RR-113 (P0 write-throughput SLO unmet), `docs/session-7.5/throughput-part2.md` (the measured
~800/s and the burst/admission evidence), `adr-multiraft-topology.md` (N), `configd-analysis.md` §3–§5.
