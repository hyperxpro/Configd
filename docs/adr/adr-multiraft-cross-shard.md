...
`adr-multiraft-topology.md` (D-B), `adr-throughput-target.md`. Evidence: `docs/multiraft/prior-art.md`,
`docs/multiraft/configd-analysis.md`. **This ADR supersedes one bullet of ADR-0023** (see Decision).
...
- **§0.2 non-goal (`PROMPT.md:35`):** Configd is **NOT** *"a multi-key transactional store (no
  cross-key atomicity beyond single-config writes)."* Scope creep requires architect + researcher
  sign-off.
- **`consistency-contract.md` §5:** *"Applications needing two keys ordered must route both to the same
...
| Secrets | one key per secret (refs a secrets manager) | No — §0.2 non-goal; rotation is per-secret |
...
### Exact consistency-contract language (drop-in for `consistency-contract.md` §5/§9)
...
> its own linearizable order, and its own applied-mutation sequence (§4).
>
> **Guaranteed:** *Per-key linearizability* — every `PUT`/`DELETE` to a key is linearizable within
> that key's shard (ReadIndex; §1). *Per-shard total order* — all writes within one shard share a
...
> consistent point-in-time snapshot; it composes per-shard cursors (§3/§6).
>
...
> Configd deliberately does not provide cross-shard transactions (§0.2); co-location is the supported
> mechanism for multi-key consistency.

## Rationale

1. **No named requirement; an explicit non-goal.** §0.2 fences it out; the contract already routes
...
3. **BUILD breaks the latency budget and re-opens consensus-grade verification.** The simplest viable
   ...
   (+2 oracle round-trips if Percolator-ordered) — over the §0.1 <150 ms budget — and introduces a
...
`RegionEpoch{ConfVer,Version}`, owned by D-B), not transactions. See `prior-art.md` §1–§4, L4.

## Rejected Alternative — BUILD distributed transactions

2PC-over-Raft / Percolator / parallel-commits / Spanner-2PC. Rejected: contradicts §0.2; no named
...
- Doc precision (not a break): the edge-staleness commit-timestamp (§2) is per-shard-leader; cross-shard
  staleness comparisons inherit per-leader NTP skew (≤50 ms) — already the only residual error term in
  §2, now across more leaders.
...
- **"ADR-0023 literally lists a cross-shard 2PC coordinator for v0.2."** *Surviving — the one genuine
  ...
  requirement, and it conflicts with §0.2 and `research.md`. This ADR supersedes it (Open Q1). Recorded,
  not averaged.
...
migration bullet), `consistency-contract.md` §5/§9, `research.md:343/385/401`, `prior-art.md` L4.
