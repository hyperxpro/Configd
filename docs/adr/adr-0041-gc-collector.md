...
- **Pause tail is the deciding axis.** A 150 ms cross-region write-p99 budget is dominated
  ...
  the commit waits on. ZGC's **max** STW pause (0.045 ms, both forks) is **~450x smaller than G1's max**
  (20.6 ms; 28.9 ms single-fork worst) and ~20x smaller than Shenandoah's max (0.905 ms). ZGC keeps the GC contribution
...
ZGC only **tightens the us/ms pause tail** that a read could ever land behind (G1's 29 ms
