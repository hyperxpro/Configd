# ADR-0040: Narrow poison-pill policy; descope architecture section 8's circuit breaker and negative caching

- **Status:** Accepted (content pre-ratified by review-architect at the C2-C5 design screen, 2026-06-11 - authored before C3 implementation per that review's hard sequencing gate)
- **Date:** 2026-06-11
- **Amends:** `docs/architecture.md` section 8 ("Negative Caching", "Poison-Pill Handling" -
  amended by reference)
- **Affects contract-test-map rows:** CT-32 (negative caching), CT-33 (poison-pill)

## Context

Architecture section 8 specifies three edge-cache defenses written for a system that does not
...
- An **apply-throwing** delta (decode/apply exception on an otherwise signature-valid
  ...
  2. **Skipping is forbidden** - a skipped seq is a silent chain break (divergence, the
     unforgivable outcome). On quarantine: emit `configd.edge.poison_pill` (the section 8 metric
...
- **Descoped (the section 8 circuit breaker):** dual-TTL, schema validation,
...
- The `BloomFilter` class (real, unit-tested, zero `src/main` consumers) is retained as
  shelfware pending an orphan sweep (delete-or-revive across all
...
- The section 8 text is amended by reference; the consolidated architecture/contract doc pass
  updates the section to point here.
- Honesty note for the register: section 8's circuit-breaker prose was another instance of the
  "documentation of another system" pattern - the descope is recorded as the
  correction, not as lost functionality.

## Sign-off

- review-architect: **pre-ratified** at the C2-C5 design screen (hard gate: this ADR authored before C3 implementation - satisfied).
