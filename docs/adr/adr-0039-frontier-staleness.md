# ADR-0039: Frontier-based staleness — fixing the idle-staleness defect in contract §2

- **Status:** Accepted (arbitrated and adopted by review-architect at the C1 design review,
  `docs/session-3/reviews/c1-design-review.md` §B-1, 2026-06-11; prod-blocking at the C2 gate:
  C2 may not wire the idle-time `StalenessTracker` as the production staleness signal)
- **Date:** 2026-06-11
- **Session:** 3 (Edge Data Plane) — written before C2 per the review condition
- **Amends:** `docs/consistency-contract.md` §2 (INV-S1 measurement definition; the threshold
...
Contract §2 / INV-S1 defines edge staleness as
...
C1 protocol design and independently verified by review-architect (review §B-1):
"an idle-but-healthy edge does walk to DISCONNECTED as written."

The root confusion: §2 conflates **"my data is old"** with **"I am behind the source"**.
...
1. The C1 fan-out protocol's `HEARTBEAT(latestSeq, serverNowMillis)` frame (cadence
   `edge.fanout.heartbeatMs` = 250 ms, 2× margin against the 500 ms STALE threshold) is the
...
3. While writes flow, applied notifications dominate the max and the measurement is exactly
   ...
   written.
4. The §2 thresholds (500 ms / 5 s / 30 s), states, behaviors (header, counter, unhealthy,
...
- **bounded by the control plane's own health surface**: the fan-out node sits on a CP node
  ...
  monitored (Session 6 wires the alerts; the metrics exist);
- the alternative — leader-signed heartbeats — would put a signing operation on a 250 ms
  ...
  without commensurate benefit. Recorded as a possible Session-5/7 revisit if the threat
  model hardens.
...
- Contract §2's measurement paragraph and INV-S1 formula are amended by this ADR (the
  consolidated contract-text pass happens at session close per the established pattern;
...
- review-architect: **ADOPTED at C1 design review** (review §B-1: "frontier-based fix is
