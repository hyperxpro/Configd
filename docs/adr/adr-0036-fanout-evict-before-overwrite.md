# ADR-0036: Amendment to ADR-0034 §5 — eviction must advance `tail` before the in-place overwrite (RR-096)

- **Status:** Accepted (Session 3; second-agent verification recorded in the RR-096 register row)
- **Date:** 2026-06-11
- **Session:** 3 (Edge Data Plane) — found on the session's first action (verify CI on the pushed branches)
- **Findings:** RR-096 (P1, FanOutBuffer torn read under eviction — RESOLVED), RR-097 (P2, session branches never pushed / CI never run)
...
ADR-0034 §5 specified the RR-066 verify-after-read protocol for `FanOutBuffer.readSince`
...
Session 2's local verification and was caught by CI. jcstress
(`FanOutBufferReadSinceTest.ExactlyFullWrap`, written in S2) detects it deterministically
in `quick` mode (14 forbidden-state observations on the dev box); gate-2 had only run the
curated subset in `sanity` mode. Captures: `docs/session-3/captures/rr-096-prefix-failure.txt`.
...
contain a lapped entry** — the property ADR-0034 §5 claimed, now actually delivered.
...
- Gate-3 adds the FanOutBuffer eviction jcstress variants in `quick` mode (gate-2's
