# ADR-0034: The commit-notification boundary (section 4.6) - bounded, cursor-based, replayable interface for the edge data plane

- **Status:** Accepted (review-architect APPROVED 2026-06-11 - sign-off recorded below; second-reviewer RR-066 co-sign rides the jcstress round).
- **Date:** 2026-06-11
- **Interacts with:** ADR-0033 (commit-outcome / applied-mutation seq S - this interface consumes that apply seam), ADR-0035 (leader-assigned commit timestamp = the section 2 staleness clock - emitted by the control plane, consumed by the edge), ADR-0030 (single-group topology - one ordered mutation stream), ADR-0028 (snapshot format - the ReplaySource payload)

## Context

section 4.6 requires a **commit-notification interface** that the eventually-consistent edge data plane consumes: versioned, cursor-based, replayable from the log/snapshot, with an explicit bounded-buffer overflow policy. The charter's premise that the buffer is *unbounded* is **incorrect** (RR-042): `FanOutBuffer` was already a bounded ring of 10,000 with drop-oldest eviction. So the deliverable is NOT "add a bound" - it is:
...
| `commitTimestampMillis` | `long` | The **leader-assigned commit timestamp** (ADR-0035 section 2): the leader's wall clock captured at apply time. This is the section 2 staleness clock - the edge computes `staleness = edge_wall_now - commitTimestampMillis`. A single authoritative reference clock (the leader's), **not** a per-entry HLC carried in the Raft log (ADR-0035 descoped that). |
...
The notification is the section 4.6 unit. **No wire encoding** lives in it - the edge implementation owns transport; the interface is transport-agnostic.
...
**Why 10,000.** At the section 0.1 baseline 10k writes/s, 10,000 entries is ~1 s of buffered tail - comfortably longer than the section 0.1 p99 propagation budget (< 500 ms), so a consumer within SLO never needs to replay; at 100k/s burst it is ~100 ms, still within the budget for a healthy consumer, and a bursting-behind consumer correctly falls back to replay rather than the cache growing unbounded. Memory: 10,000 references + small per-notification objects - negligible. No argument to change it.
...
`version` is the ADR-0033 applied-mutation seq S (the listener fires only on mutating applies). `clock.currentTimeMillis()` here runs on the **leader's** apply path, so it IS the leader-assigned commit timestamp ADR-0035 section 2 redefined - captured **without editing `RaftNode`** (the state-machine listener wiring is the boundary-engineer's to extend; the apply seam already exists). The buffer is constructed with a `FanOutMetrics` sink bridging to the `fanout.buffer.dropped` counter. `ConfigdServer.commitNotificationSource()` and `replaySource()` expose the seams to the edge. **No fan-out, no subscription, no wire protocol, no edge code added** - the named overflow/gap/replay seams are the control-plane deliverable.

## Rejected alternatives

- **Block / backpressure the apply thread on a slow consumer.** Rejected - the apply thread is the single-writer Raft commit path (R-01); blocking it on a data-plane consumer couples control-plane liveness to edge health and violates section 0.1 write-commit latency. Drop-oldest + replay decouples them.
...
- **Lock the buffer for read/write.** Rejected - section 5/section 6 hard rules forbid locks on the read path; the verify-after-read is lock-free and the append stays allocation-free.
...
- **Carry only (seq, key, value) and drop the delta's signature/epoch/nonce.** Rejected - the edge needs the F-0052 signature/epoch/nonce to verify authenticity and reject replays; stripping them would force the edge to re-fetch or trust unsigned data.
...
- **Positive:** RR-042 RESOLVED (read side is consumer-facing with a safe contract); RR-066 RESOLVED (race closed where it activates); RR-001 blast radius reduced (the bounded, replayable, race-safe interface now exists - only the wire/edge path remains). ADR-0035's section 2 staleness clock is now emitted on the stream. The schema is transport-agnostic, so the edge implementation is free to choose its wire format.
...
3. **Wire / transport (edge implementation owns).** This interface is transport-agnostic. Build the push/pull transport, subscription model (per-key/prefix/full-store), and backpressure on top of `readSince`/`ReplaySource`. **Never** consume `FanOutBuffer.deltasSince` (non-atomic, RR-066) - only `readSince`.
4. **Edge staleness (ADR-0035 section 2, joint with this work).** Feed `commitTimestampMillis` into `StalenessTracker.recordUpdate(version, timestamp)` (make the currently-ignored `timestamp` param load-bearing); compute `stalenessMs = edge_wall_now - commitTimestampMillis`. Assert INV-S2 (p99 < 500 ms / p9999 < 2 s) in `StalenessUpperBoundTest`. Document the <= 50 ms NTP-skew assumption + a negative/implausible-staleness tripwire.
...
- review-architect: **APPROVED 2026-06-11.** Verified all four points. (1) **RR-066 memory-ordering - sound.** The verify-after-read (read tail t1 -> head h -> copy -> re-read tail t2; GAP if `t2!=t1`), the `h-t1>capacity` lap case (prevents a wrapped slot being visited twice = a duplicate), and the `lastEvictedSeq`-before-`tail` publish order (a reader observing the advanced tail also observes the watermark -> GAP, never a truncated run) together make every non-GAP run provably strictly-ascending, contiguous, no-dup, no-skip, no-torn. The exactly-full `(head-tail) >= capacity` eviction guard is correct (a `>` would silently overwrite the oldest at full occupancy). Confirmed via adversarial analysis + `FanOutBufferRaceTest` (200k writes / 4 readers / cap 64 + reader-paced exactly-once). (2) **Schema carries exactly what S3 needs and no more** - `seq` (ADR-0033 cursor key), `commitTimestampMillis` (ADR-0035 section 2 staleness clock), `delta` (mutations + F-0052 signature/epoch/nonce for edge verify); no wire encoding (transport-agnostic). (3) **Drop-oldest + replay loses no committed effect** - `replayThenTailObservesEveryMutationEffectExactly` (25 seeds x randomized overflow/replay/tail) asserts the consumer's materialized view byte-equals the authoritative cumulative state and the cursor reaches the authoritative seq; non-vacuous (real key/value equality). The snapshot-equivalent (not full-historical) replay is honestly characterized in section 4 (the edge applies cumulative state; an auditing consumer would get a separate WAL-backed seam - out of scope). (4) **No fan-out / wire / edge code crept into S2** - only the named overflow/GAP/replay seams + producer wiring; subscription/transport/edge are explicitly S3. No required changes.
