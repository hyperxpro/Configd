...
- Contract section 2 defines edge staleness as `current_wall_time - timestamp_of_last_applied_entry` and states
...
- Contract section 4 promises "Each entry also carries an HLC timestamp for cross-group approximate ordering"
...
1. **Cross-group approximate ordering** (section 4 `:119`, section 5.3 `:145-148`, INV-W2 `:156`, section 9
...
- **Replace** section 2 "Staleness Measurement" body:
...
### section 4 - Version Semantics (`consistency-contract.md:117-119`, INV table `:108-114`, section 5.3/INV-W2, section 9)

- **section 4 "Semantics"** (`:117-119`):
...
- **section 4 comparison table** (`:108-114`): the "Cross-key ordering / Vector Clocks ... Causal only" row stays;
...
- **section 5.3 Cross-Key Order Across Raft Groups** (`:145-148`): change "Cross-group ordering is approximate
...
**Proposed replacement for section 4 "Semantics" seq bullet (`:118`):**
...
>   (section 3/section 6). It is gap-free **over the mutation stream**, not over raw log indices.

**Proposed replacement for INV-V2 (`:131-132`):**

> INV-V2: for all  Raft group g, for all  consecutive **mutating** committed entries e_i, e_{i+1} in g (i.e., adjacent
...
> commit-notification timestamps (ADR-0035 section 2 amendment), not a per-entry HLC. The sequence number is now
...
- **Keep section 2's current local-idle-time proxy and just relabel the contract to match the code.** Rejected as
...
## What the edge data plane must implement for staleness measurement

These are owed by the edge data plane implementation:
...
(Note: ConsensusSpec models `EdgeApply(e)` as an
