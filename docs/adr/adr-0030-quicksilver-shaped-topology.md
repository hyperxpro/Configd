...
> and **reject global multi-region / hierarchical Raft *write* consensus** (the section Reasoning latency
...
>   aggregate throughput is **unmeasured pending the EC2 Nxknee measurement**.
> - **The async fan-out is now WIRED (the edge data plane).** The point-7 / Consequences residual
...
>   `consistency-contract.md section 8`). The "no runtime invariant enforcement in production" risk is likewise
>   closed - `InvariantMonitor` runs in production (`testMode=false` => metric + log; `consistency-contract.md section 8`).
> - **The section 0.1 write-availability renegotiation is resolved:** `adr-0031` is now **Accepted**, so the
>   "HUMAN RATIFICATION PENDING / stops for review before merge" caveat under section "SLO impact" is discharged.
...
section 0.1 (`PROMPT.md:16-27`) fixes the contract for any topology:
...
(confirmed by code inspection). The async fan-out half is unwired:
...
nothing outside the class - verified by grep), and `PlumtreeNode.broadcast()` is invoked only
in a benchmark.
...
and explicitly defers data residency with the residual stated (section 0.2 does not enumerate residency
as a non-goal; A3 defers it rather than claiming section 0.2 scoped it out).

The section 0.2 non-goals (`PROMPT.md:31-41`) fence this: Configd is not a general KV store, not a
...
consensus" arguments assume transactional or residency semantics that section 0.2 places out of
scope.
...
commit floor low (intra-region AZ RTTs are single-digit ms, far under the section 0.1 budget) while
...
directive given for this ADR (residency is not enumerated in section 0.2's non-goals; it is deferred
here, not declared out of scope by section 0.2), with the residual stated. The
...
- **etcd / single-group Raft (`PROMPT.md:111`).** Borrowed
...
All external numbers below are drawn from published primary sources (Cloudflare engineering posts, etcd documentation, CockroachDB/Spanner papers). All repo numbers carry `file:line`. Numbers I could not measure in this repo are labeled **modeled** or **unmeasured**.

### 1. Cross-region write consensus cannot meet section 0.1's `write p99 < 150 ms` for a non-co-located client.
...
- But section 0.1's budget is **cross-region**, i.e. it must hold for a writer not next to the leader.
  End-to-end = `RTT(client<->leader) + commit_floor`. For an **ap-southeast-1 client to a us-east
  group: 219.64 + 68.55 ~ 288 ms at the floor** - **~1.9x over the 150 ms budget**, before
  fsync (etcd targets each node's fdatasync **under 10 ms**), batching, and queueing.
  ap-northeast-1 ~ 217.7 ms (~1.45x over).
...
- **Write p99 < 150 ms:** commits occur inside one low-RTT cluster (intra-region AZ RTTs are
  ...
  choose a far smaller micro-batch. **Modeled**.
- **Propagation p99 < 500 ms global:** asynchronous fan-out. Quicksilver publishes the
  ...
  `[UNVERIFIED for Quicksilver]` in the research.
...
### 4. It also wins section 0.3's operational-complexity axis.
...
with monotonic sequence numbers for gap detection. section 0.3 grades operational complexity as "zero
...
arguments **in their strongest form**, then rebuts each. Where a
...
**Their argument:** the Quicksilver shape is being
...
not use it as one. The verdict in section Reasoning rests entirely on (a) measured cross-region RTT
arithmetic vs section 0.1, (b) etcd's own write-scaling constraint,
and (c) Cloudflare's published existence proof that the
shape meets the budgets at 330-city scale. If the code
registered five cross-region groups today, the section C arithmetic would still reject them. The
...
*Their strongest form:* one group, one leader, one root
...
namespace) stops **100% of writes worldwide** until a human rebuilds/fails over. section 0.1 demands
...
- The adversary's premise that the root is necessarily a **single failure domain** is the part
  ...
  write quorum and **without** the ~288 ms latency penalty of section Reasoning-1. Commit floor stays
  single-digit ms (intra-region AZ RTT), well under section 0.1.
...
*Their strongest form:* a revocation (`auth.acl.revoke`,
...
- **Amendment A1** introduces a `GLOBAL`/security key class that is **exempt from bounded-stale
  ...
  breach" capability the adversary demands - obtained **on
...
*Their strongest form:* section Reasoning-1's 288 ms figure is a
...
- **The "100x regression on regional writes" assumes the writer is far from the root; that is a
  ...
  changes (Quicksilver's 2020 write rate was ~350/s, baseline section 0.1 is 10k/s),
  **not** latency-sensitive per-request writes from
  ...
  not a high-rate per-region application-write load. (section 0.2 does not enumerate a "regional write
  store" non-goal; the point rests on the write-population profile, not a section 0.2 fence.)
- **Decisive point: the adversary's own ~9 ms average is dominated by the in-region commit - i.e.
  ...
  one-per-region without a shared order - and per their own #3(c),
  ...
  (section Reasoning-4) to manage the fragmentation. For a 10k/s operator-driven write load, one ordered
...
ordered root meets section 0.1 and avoids the fragmentation.**
...
*Their strongest form:* GDPR/Schrems II, India DPDP/RBI,
...
ZK-shaped external coordination `PROMPT.md section 5 rule 5` forbids and destroys shared ordering.
...
- This is a genuine constraint that the Quicksilver shape does **not** solve, and I do not claim
  ...
  the residual stated"; section 0.2 (`PROMPT.md:31-41`) does **not** enumerate residency as a non-goal,
  so I am **deferring** it (Amendment A3), not claiming section 0.2 scoped it out.
...
not declared out of scope by section 0.2, which does not enumerate residency. Mitigation: N
...
*Their strongest form:* with all voters in one failure domain,
...
TLA+-green Leader Completeness), and zero acknowledged-write-loss.
...
- **Acknowledged-write durability is preserved by *acking only on root-quorum durability*, not on
  standby replication.** The delta-loss the adversary describes exists only if the system acks before
...
(point 1) and (b) the async-unreplicated write tail delta is lost (ACCEPTED TRADE-OFF). Mitigations:
...
*Their strongest form:* one write group = one
...
**~10k commits/s** - exactly the section 0.1 *baseline* with **zero** headroom for the **100k/s burst**;
...
- **The "looks done" trap is real and accepted** - addressed in the framing rebuttal: this
  decision is justified by section 0.1 arithmetic, not by code shape. Adopting this ADR does **not** mean
...
*Their strongest form:* the async-fan-out half does not run -
...
bound don't exist / lack a history checker; the perf scorecard is
...
- **Conceded, on the record (`file:line` verified in this repo).** `fanOutBuffer.append(delta)` at
  ...
  `< 500 ms` propagation number in section Reasoning-3 is marked `[UNVERIFIED for Quicksilver]` and
...
- **The adversary's "extend the verified-real core across regions" is the part the section C arithmetic
  refutes (point 3 / section Reasoning-1):** the core is verified as a *single group* (`ConsensusSpec`);
  multi-region/hierarchical Raft is unbuilt and unverified.
  "Extending it across regions" is **new, unbuilt, unverified work** that
  ...
  closed-timestamp side-transport) stands on section Reasoning-4.
...
"meets section 0.1 propagation" or "contract verified" claim is made. Until measured, all propagation and
...
- **Hierarchical Raft (global group + regional groups) - `adr-0015`'s design:** the *global* group
  ...
  PlacementDriver + closed-timestamp side-transport complexity (section Reasoning-4) that loses section 0.3's
...
- **Meets section 0.1 write p99 (modeled):** commits intra-cluster in one low-RTT region; no APAC writer
  pays the ~288 ms cross-region floor.
- **Meets section 0.1 edge read p99 < 1 ms / p999 < 5 ms (read path verified real):** local
  lock-free MVCC serving with no consensus (`VersionedConfigStore.java:42, :189`),
  matching Quicksilver's published "90% < 1 ms".
- **Wins section 0.3 operational complexity:** one root + fan-out tree vs PlacementDriver +
  closed-timestamp side-transport (section Reasoning-4); "zero external coordination" (`PROMPT.md:54`).
...
- **Data residency unsolved** (point 4 RESIDUAL): single-root deployment non-compliant for
  hard-localization data classes; deferred with residual stated (A3) - not scoped out by section 0.2.
...
- **Unwired fan-out (highest):** mitigation - wire `FanOutBuffer` drain -> `PlumtreeNode.broadcast`
  ...
  history checker **before** claiming section 0.1 propagation is met.
- **Thread-unsafe `RaftNode`/`ConfigStateMachine` integration race**
  ...
  executor; required under any topology.
- **No runtime invariant enforcement in production** (`ConfigdServer.java:248` NOOP):
  mitigation - wire a real `InvariantChecker` before GA.
...
### SLO impact - section 0.1 write-availability (KNOWN VIOLATION under full-region loss)

- **The single-region root, even with A2's multi-AZ spread, does NOT meet section 0.1's 99.999%
...
- **Edge READS (section 0.1's 99.9999% target) are UNAFFECTED by root-region loss** - they are served
  from local edge copies that do not depend on root liveness.
- **section 0.1 (`PROMPT.md:27`) requires an ADR to change a target.** This is therefore recorded as a
...
- **Triggers `adr-0031-renegotiate-write-availability-target.md`** (stub, Proposed): section 0.1 requires
