...
- **Status:** Accepted (Netty-migration, operator architectural decision). Staged by verification -
  each surface migrates + re-proves its S7 controls behind its own CI-green gate.
- **Date:** 2026-06-23
...
- **Evidence:** the built-and-raced head-to-head benchmark (independently reproduced); API research in the Netty 4.2 API study.
- **Decision log:** the Netty migration decision log.
- **Migration status (2026-06-25): COMPLETE - all four surfaces migrated.** M1 edge-read + M2 admin
  ...
  artifact) because the send is driven from the event loop, exactly as this ADR's consensus
  ...
  the io_uring rationale is **measured as NOT validated** - latent at best, a ~2x throughput
  regression at high fan-out - see the honest-rationale point 1 below and the io_uring measurement
  report. The migration stands on its other two
  measured grounds (edge-read allocation 8.7x + uniformity); io_uring stays an auto-selected tier with
...
1. **io_uring (the platform unlock - the *bet*, now MEASURED in Phase V: NOT validated).** Netty 4.2
   ...
   **syscall reduction** via batched submission/completion rings - an axis the allocation
   ...
   independently reproduced - see the io_uring measurement report).
...
- **Auto-select Epoll -> NIO; io_uring opt-in, fail-loud, fallback CI-proven.** io_uring is a
  ...
  V, *faster*) default the CI runners exercise. *(Originally io_uring -> Epoll
...
- **mTLS / S7 surface (preserved, re-proven where it exists).** The S7 controls are already
  ...
  property of the **fan-out** surface (M3), not M1. M1 re-proves the
