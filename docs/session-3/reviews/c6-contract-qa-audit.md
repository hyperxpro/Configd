...
# ADDENDUM — Final flips at the gate-3 landing (2026-06-12)

> **Scope:** the gate-3 landing (`31a4225`, "C6 SIGNED OFF — RR-001 RESOLVED ... gate-3
> assembled") plus the uncommitted-at-this-pass working set it points at:
> `gates/gate-3.sh` end-to-end GREEN captured at
> `docs/session-3/captures/gate-3-run.txt` (all 8 steps OK, `ALL STEPS GREEN`),
> `EdgeMetricsContractTest` (new, configd-edge-node),
> `ProbeMechanismTest#reportEmitsOneGreppableProbeHistogramLinePerScope` (new test in the
> existing suite), the regenerated `captures/ct34-jmh-gc-check.txt`, and
> `docs/session-3/handoff-to-session-4.md`. This addendum resolves the three open items
> of this audit's end-state sweep and supersedes its three-violation verdict.
>
> **Evidence discipline:** `gate-3-run.txt` read in full; `EdgeMetricsContractTest`
> read in full (the checklist-disposition javadoc is the load-bearing artifact);
> the probe format test read; the regenerated gc artifact's provenance header checked
> (UTC 10:57:09Z, git `31a4225`, gated legs named); `gates/gate-3.sh` steps (b)/(d)/(g)
> read at source; handoff §3 grepped for the CT-02 naming; the `c9751d8` diff read
> (sweep finding 4 follow-up). **My own re-run** (box idle-checked: pgrep CLEAR, no
> containers): `EdgeMetricsContractTest` 6/6 + `ProbeMechanismTest` 4/4 green.

## CT-34 — PARTIAL(unit) → **PASSING** ✅ FLIPPED

The row's stated remainder was exactly "gate-3.sh invokes `jmh-gc-check.sh` + artifact
cited mechanically". Both are now true inside a CAPTURED run: step (g) executes
`gates/jmh-gc-check.sh`, the capture shows "GATE-3 gc: OK (CT-34: zero steady-state
allocation on the edge read path, artifact saved)" with the gated legs green (getMiss
≈0 B/op, getIntoHit 0.002 B/op — both under the < 1 B/op gate; getHit legs
captured-for-trend per the unchanged discipline), and the artifact was REGENERATED by
that run with its provenance header (date/git-SHA/gated-vs-trend statement). The C2-era
scope ruling (law binds the in-process path; HTTP shell priced out) was accepted at the
C2 sign-off (Finding 4, non-blocking) — nothing left open. Flipped with capture +
artifact citations in the row.

## CT-38 — PARTIAL(unit) → **PASSING** ✅ FLIPPED (deviations priced, not waived)

The owed consolidated gate exists and is genuinely consolidated: 53 exported series as
string literals (the machine-checked "named list in handoff" — Session 6's dashboard
contract; a rename breaks the build loudly), presence on the FIRST zero-traffic scrape
over both halves (RR-013), exact sample-line matching (no `# TYPE`/prefix false
satisfies), and one emission movement per checklist family through real seams. The two
non-literal checklist items are priced 1:1 in the test's own checklist-disposition
javadoc, which I verified says what the flip requires:

- **Staleness HISTOGRAM:** a deliberate deviation, not silent closure — no Prometheus
  histogram series exists by design; the distribution obligation is met by the charter
  §3 V2 probe's `PROBE-HISTOGRAM` output, now emitted in ALL modes inside the captured
  gate (step (d) runs sim + live boundary + live EDGE and greps the
  `scope=global` lines), with the report FORMAT pinned by the new ProbeMechanismTest
  assertion. The cross-module placement is justified at source (testkit depends on
  edge-node; a direct reference from EdgeMetricsContractTest would be a cycle — the
  javadoc cites this). A histogram-as-SERIES remains Session 6's backend call, recorded.
- **Per-subscriber queue depth:** the established label-free-registry deviation
  (process-level high-water gauge), carried since C1, now asserted in the gate
  (high-water semantics: 7 survives a later 3).

V2 probe histograms — the row's last open checklist item — are DELIVERED: the live EDGE
mode (the Phase-V capture's "lands with C6" promise, flagged by this audit's first pass)
landed at the gate-3 window and ran green through the real wire path. Flipped with the
deviations recorded verbatim in the row note.

## CT-02 — stays PARTIAL(unit); deferral SANCTIONED on the record

As this audit's end-state sweep asked: the row note now records that the deferral is
sanctioned by charter §3 V2's own text ("Session 5 will use this for the real
p99 < 500 ms target; here it must merely work and produce honest numbers on throttled
hardware, recorded with caveats") and NAMED in `handoff-to-session-4.md` §3 ("CT-02
sanctioned deferral (S5)"). The vehicle is complete and gate-run — the live edge mode's
honest throttled-box numbers (p99=4871 ms on a loaded 2-vCPU host, poll-granularity
sampling) are exactly the charter's "honest numbers with caveats" and exactly why the
bound is S5's to measure. The gate-3 end-state (one PARTIAL row) is justified without an
ADR; the doc-pass ledger now carries the contract:214 wording amendment.

## Sweep follow-ups closed

- **Finding 4 (modified cited test):** `EdgeBootstrapUnderSustainedWritesProcessTest`'s
  uncommitted modification landed as `c9751d8` — the paced leg's `snapshotsApplied`
  relaxed `==1` → `1..2` against SIGNED-OFF F3 behavior (post-cutover ack-RTT redundant
  idempotent re-demote; a CI-flake over-pin, the session's first CI red). Verified at the
  diff: leg 1 still pins exactly 1; every torn/restarted-transfer discriminator is an
  unchanged exact zero; `snapshotsApplied` was never the torn-transfer discriminator.
  CT-24's row carries a dated consistency note; claims stand, no flip.
- **`gates/gate3-map-expectation.txt`:** updated in the SAME pass as the map's summary
  line (gate-3 step (b)'s paired-change rule), to
  `total=41 passing=36 partial=1 failing-captured=0 unimplemented=0 adr=3 na=1`.
- **CT-39 honest-scope refresh:** the E2E scenario is now twice-captured (the gate-3 run
  repeated it 19/19 GREEN, step (f)) — the "one captured run" caveat is retired in the row.

## Final end-state verdict (supersedes the sweep's three-violation table)

**40 of 41 rows PASSING / ADR-renegotiated / N-A; exactly one row (CT-02) PARTIAL as a
sanctioned, named, handoff-recorded S5 deferral.** No row violates the charter §3 V3
end-state silently: the single deviation is on the record in the charter's own V2 text,
the handoff, the row, and this audit. Gate-3 step (b) enforces exactly this state
mechanically (structural zero-FAILING/zero-UNIMPLEMENTED checks + the exact-summary pin).

## Defects found at this pass

1. **None blocking.** The consolidated gate's emission checks are deliberately light
   (presence + one movement per family) — correct layering, since per-series semantics
   are pinned in the named per-component suites; the javadoc says so explicitly.
2. **Cosmetic, not tracked:** the gate capture's probe edge-mode p99 (4871 ms) will
   alarm a casual reader without the in-capture caveat lines; the caveats ARE printed by
   the probe itself (hardware + sampling), so no action — noted here for S5's reader.

## New summary line (recounted: 36 + 1 + 0 + 0 + 3 + 1 = 41)

```
CONTRACT-MAP-SUMMARY: total=41 passing=36 partial=1 failing-captured=0 unimplemented=0 adr=3 na=1
```

Map and `gates/gate3-map-expectation.txt` carry the identical line, updated as one pass.
