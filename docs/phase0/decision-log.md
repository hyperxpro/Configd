...
|---|---|---|---|---|---|
| D-006 | 2026-06-21 | technical | The R-01′ owner-thread tripwire reuses `RaftNode`'s existing `InvariantChecker` seam (`:219–224`) — throws in test/sim, metric+SEVERE in prod — mirroring `ConfigStateMachine.assertOwnerThread()` (RR-029/W-1). | The seam already drives the 9 in-node invariants; reusing it means zero new verification plumbing and identical test/prod duality. | Yes (impl detail). |
