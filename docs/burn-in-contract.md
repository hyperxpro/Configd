...
> **Status: DRAFT for operator adoption.** This document closes **condition C4** of the v1
> `GO-WITH-CONDITIONS` ship gate (`docs/readiness/v1-go-no-go-2026-07-01.md` §0). It is a posture
> the operator *adopts and operates*; the reliability engineer produces the document, the operator
> owns the rotation, the ratification, and the exit call.
>
...
OOM'd at **3.45 h** (RR-112 = box capacity on that run, *not* a leak; the clean-code soak reached
...
- **Horizontal scale** — `ec2-horizontal-2026-07-01/02-scaling-curve.md` + `04-verdict.md`:
  near-linear **2.45× on 3 machines** (656 → 1075 → 1607 w/s), cluster-bound by consensus churn, not
  hardware.
- **Single-node ceiling** — `ec2-2026-06-30/01-nxknee.md`: **~800 w/s open-loop knee (RR-113)**,
...
| Leadership | **Operator-managed, not auto-balanced.** The 2.45× requires one-leader-per-box; `transferLeadership` exists in core but is not exposed on any admin route and not invoked on shutdown (go/no-go §3.2). |
...
table (§6). Series carry no app-level `instance`/`cluster` labels — those come from the scrape config.
...
| **Raft apply backlog / follower lag** | Apply backlog ~0 steady-state; recovery RTO 4.2 s / 5.9 s (`02-dr-drills.md`) | `max(configd_raft_pending_apply_entries) > 5000` for 5m (existing `ConfigdRaftApplyBacklog`); per-shard `raft_shard_apply_lag_<gid>` rising-and-not-recovering. | A leader that commits but cannot apply, or a follower that never catches up, shows here. **FLAG:** there is **no true per-follower matchIndex-vs-leader-commitIndex gauge** emitted (see the snapshot row and §6); this is the closest real proxy. |

### 2C. The correctness / security / snapshot / edge class

| Signal | Measured baseline (source) | Heightened alarm | Rationale |
|---|---|---|---|
| **Snapshot size vs the 4 MiB cap** | Cap `MAX_SNAPSHOT_BLOB_LEN = 4 * 1024 * 1024` (`RaftMessageCodec.java:76`); an over-cap InstallSnapshot frame is dropped to stderr and the follower **wedges** (`RaftNode.java:2074-2077`) | (a) **Log-alert** on the stderr string `snapshot too large for v1 wire` → **any occurrence = PAGE**; (b) existing `ConfigdSnapshotInstallStalled` `increase(configd_snapshot_install_failed_total[15m]) >= 3`; (c) the apply-lag proxy above. | **FLAG (load-bearing):** there is **no snapshot-size gauge** — "approaching 4 MiB" cannot be a metric alarm; it is **capacity discipline** (keep `ConfigStateMachine.snapshot()` state < 4 MiB, known-limitations.md §"Snapshot size cap"). And the **leader-side >4 MiB drop increments no metric at all** (a bare `System.err.println`) → the log-watch is the *only* direct signal. `configd_snapshot_install_failed_total` counts follower-side *apply* failures, not the leader drop. **Recommend a snapshot-bytes gauge + a matchIndex-lag gauge as the first burn-in instrumentation.** |
| **ACL policy load failed** | Healthy = 0 (`configd.acl.policy.load.failed`, `ConfigdMetrics.java:90`) | `increase(configd_acl_policy_load_failed_total[15m]) >= 1` for 5m **WARN** (existing `ConfigdAclPolicyLoadFailed`) → investigate immediately; if it persists while `configd_acl_policy_reload_total` does **not** advance ⇒ last-good frozen (poison key) → escalate to the rollback check (§3). | A failed load = an `_acl/` update **silently did not apply**; the loader fails closed to last-good, so clients see no error. A poison key delivered via snapshot/replay **freezes every subsequent policy update** until removed → security drift. |
| **Leadership distribution (N>1 only)** | 2.45× **requires** 1-leader-per-box (`02-scaling-curve.md`); leaders can "sweep" onto one node; no auto-balancer; **2-1-0 tolerated** (1628 w/s) (`05-leadership-placement.md`) | Alarm if `max(raft_node_leader_count) - min(raft_node_leader_count)` exceeds modest imbalance sustained — e.g. one node leads **all** N groups while another leads **0** (full sweep). | **FLAG:** only material at **N>1** — v1 default is N=1, so this is dormant unless the operator runs sharded. The **response** is a manual / rolling-restart re-spread; `transferLeadership` is not exposed (go/no-go §3.2). Robust to 2-1-0; a full sweep forfeits the horizontal benefit until restart. |
| **Edge staleness** | Contract §2 state boundaries: CURRENT→STALE **500 ms**, STALE→DEGRADED **2 s**; S5 reference p99 **255 ms** (`configd-slo-alerts.yaml` comment) | Existing `ConfigdEdgeStalenessWarn` `max(edge_staleness_ms) > 500` for 2m / `ConfigdEdgeStalenessDegraded` `> 2000` for 1m; watch `configd_edge_staleness_violation_total` increments and `edge_staleness_implausible_total` (clock skew, existing `ConfigdClockSkewSuspected`). | The 500 ms / 2 s boundaries are **contract-defined, not empirical**. **FLAG:** edge staleness was **not re-measured under load** in either EC2 run (both were write-plane) — the 255 ms baseline is the S5 reference. Burn-in must **capture the real production edge-staleness distribution** to calibrate (this is a direct input to closing register 7.12). |
| **Correctness invariants** | 0 violations expected (`invariant.violation.*`, `InvariantMonitor.java:43,234`; consistency-contract §8 INV-M1 / INV-S1) | Any `increase(invariant_violation_monotonic_read_total) > 0` or `invariant_violation_staleness_bound_total > 0` = **correctness-drift INVESTIGATION** (RR-099: **not** a standalone page); a confirmed violation under normal operation = rollback-consideration (§3). | These bridge the two data-plane consistency invariants into metrics. A real violation means the consistency contract itself is breaking — the highest-severity class of drift. |
| **Control-plane write availability** | SLO **99.999 %** over 30 min (existing `ConfigdControlPlaneAvailability`) | Keep as-is (page). In burn-in, review the burn against budget **daily** (§4). | The denominator is failed+total so a full outage still pages (no NaN). This is the top-line control-plane SLO for the daily error-budget review. |
...
|---|---|---|---|
| **R1** | **Heap / RSS drift past the soak envelope (OOM class)** | Post-GC heap **floor** (`min_over_time(jvm_heap_used_bytes[6h])`) rising monotonically past ~2× the ~400 MB soak floor **AND** container RSS climbing >10–15 % past its plateau and continuing toward the container limit — i.e. a *rising trend*, not a sawtooth peak. | The soak proved a **stable floor** (~390–404 MB) and a **2.6 % RSS spread** over 6 h. A monotonic climb is the exact 3.45 h-OOM signature re-emerging (`04-soak.md`). Roll back before the container OOM-kills. |
...
1. **The OOM/leak trio** — heap-used floor trend, container RSS trend, `process_open_fds` slope
   (§2A). These re-open the one risk the soak closed.
2. **Leader-churn / election rate** — `configd_raft_elections_total` (§2B). The earliest knee signal;
   the system churns before it slows, and it churns at low CPU.
3. **Commit p99 drift** — the 25 ms early-warning, well inside the 150 ms SLO (§2B).
4. **Write availability + shed rate** — the top-line SLO and the 0-reject baseline (§2B, §2C).
5. **Correctness invariants + edge staleness** — any nonzero invariant counter, and the real
   production staleness distribution being captured for calibration (§2C).
6. **Snapshot log-watch + ACL load state** — the two silent-failure classes (§2C).
7. **Leadership distribution** — only if deployed at N>1 (§2C).

**Daily error-budget review** (the C4 cadence): once per day, walk the burn-rate on the three SLO
families — control-plane write availability (99.999 %), write-commit p99 (vs 150 ms), edge staleness
(vs the §2 boundaries) — and record whether any budget is burning faster than 1×. This is the ritual
...
contract can only watch by proxy or by log today (§2C snapshot row, §6). File these as the first
...
2. **The three SLO families held** (write availability 99.999 %, commit p99 inside budget, edge
   staleness inside the §2 boundaries) with no sustained budget burn.
...
- **`docs/readiness/v1-go-no-go-2026-07-01.md`** — the ship decision this contract closes: **§0 C4**
  (the condition), **§3** (the empirical verdict every threshold is grounded in), **§5.1** (the
  operability caveats: burn-in posture, leadership-not-auto-balanced, snapshot 4 MiB cap, alert
  thresholds not calibrated).
- **`docs/operator-runsheet.md`** — the **C1** secure-by-config release gates (Auth · mTLS · Audit ·
...
- **`docs/deployer-must-know.md`** — the **C2** deliverable: the five deployer MUST-KNOW requirements
  (don't-store-secrets, legacy-SUBSCRIBE segregation, `scope`≠isolation, snapshot < 4 MiB, leadership
  placement) the server does not enforce for you. Its snapshot-4-MiB and leadership items feed directly
  into §2C / §3 of this contract.
- **Measurement docs (the grounding):**
  - `docs/measurement/ec2-2026-06-30/04-soak.md` — the leak/OOM/GC/FD/heap/commit-p99 baselines.
  - `docs/measurement/ec2-2026-06-30/01-nxknee.md` — the ~800 w/s churn knee + election-churn baseline.
  - `docs/measurement/ec2-2026-06-30/02-dr-drills.md` — durability, 372 ms failover, 0-loss, RTO.
  - `docs/measurement/ec2-horizontal-2026-07-01/02-scaling-curve.md` + `04-verdict.md` — the 2.45×
    curve, per-node CPU, leadership requirement.
  - `docs/measurement/ec2-horizontal-2026-07-01/05-leadership-placement.md` — the leadership-balancing
    operability gap.
...
- **Runbook responses:** `ops/runbooks/` (`release.md`, `resource-leak.md`, `raft-saturation.md`,
  ...
  `disaster-recovery.md`, `restore-from-snapshot.md`) and `docs/runbooks/` (`leader-stuck.md`,
  `write-freeze.md`, `cert-rotation.md`).
- **`docs/known-limitations.md`** — the snapshot 4 MiB cap section (the `matchIndex`-lag monitoring
  recommendation this contract implements-by-proxy and flags for instrumentation).
...
| `configd_raft_elections_total` | counter | `ConfigdMetrics.java:62`; incremented `ConfigdServer.java:1077` | **the churn signal.** Group-0-scoped at N=1 (go/no-go 2.8) |
...
`configd-server/src/main/java/io/configd/server/RaftMessageCodec.java:76`. Per-blob; the combined
InstallSnapshot frame must also fit `FrameCodec.MAX_FRAME_SIZE = 16 MiB`
(`configd-transport/.../FrameCodec.java:104`). Over-cap frames are dropped at
`RaftNode.java:2074-2077` with the stderr string `snapshot too large for v1 wire` and **no metric
increment**. Matches `docs/known-limitations.md` §"Snapshot size cap" and go/no-go §5.1.

**Signals with NO real emitted series (watch by the stated means, not an invented metric):**
- **RSS / container working set** → node/container exporter (`process_resident_memory_bytes` / cgroup
  memory). `JvmMetrics` exposes heap, not RSS.
- **Per-node CPU** → node/container exporter. Not app-emitted.
- **Snapshot size approaching 4 MiB** → no gauge; capacity discipline (state < 4 MiB) + the
  `RaftNode.java:2075` log-watch. **Recommend adding a snapshot-bytes gauge.**
- **Per-follower matchIndex-vs-leader-commitIndex lag** → no gauge; use the `configd_raft_pending_apply_entries`
  + `raft_shard_apply_lag_<gid>` proxies. `docs/known-limitations.md` recommends the true gauge;
  v1 does not emit it. **Recommend adding it.**

---

*Draft produced 2026-07-01 by the reliability engineer to close condition C4. Docs-only; no production
