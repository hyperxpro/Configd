...
- **Leadership balancing.** Multi-machine horizontal scale (the proven 2.45x across three machines) needs one group-leader per box, but v1 has no balancer. `RaftNode.transferLeadership` is now exposed on an ADMIN-gated admin route (`POST /v1/admin/groups/<id>/transfer-leadership`), but there is still no transfer-on-graceful-shutdown and no automatic balancer. The remaining work is transfer-on-shutdown or an automatic balancer. Until then, N=1 is the default and N>1 leadership is operator-managed.
- **Leadership transfer status read.** An ADMIN-gated per-group leadership status read (GET) to confirm the asynchronous `transfer-leadership` took effect and to detect drift; today an operator must re-read `configd_raft_role`. Deferred from the transfer gate: a GET at `/v1/admin/groups/<id>/leadership` collides on suffix with the existing `POST .../transfer-leadership` (which also ends in `/leadership`), so routing must match the transfer suffix first or use a distinct path. Read-only, off-owner-safe (`leaderId()`/`role()`), reusing the same ADMIN gate.
- **Transfer-abort metric.** The Raft dissertation section 3.10 transfer-abort (a stalled leadership transfer auto-aborting after about one election timeout so the current leader resumes) logs to stderr only. A `raft.leadership.transfer.aborted` counter would let monitoring alert on transfer churn. Requires a `ConfigdMetrics` catalog entry plus `EdgeMetricsContractTest` registration (the anti-blind-dashboard gate).
