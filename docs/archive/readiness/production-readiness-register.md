...
> **Purpose.** The single subsystem-organized go/no-go register for Configd. Every item carries an
> audited status backed by a named artifact (class:line / test / CI gate / Accepted ADR / measured
> number) or a named gap. This register drove the production-readiness review; it is meant to reflect
...
> **Note:** these counts are the snapshot as of the original audit (2026-06-26). They were not re-tallied
> for subsequent row updates -- in particular the watch rows (§4.8/6.6, §11.8) were upgraded partial/
> unknown to done (N=1, server-side) on 2026-06-29, so the live done/partial split is ~3 items better than
> shown. See the individual rows for current status.
>
> **Current reconciled status (authoritative): [`v1-go-no-go-2026-07-01.md`](v1-go-no-go-2026-07-01.md).**
> The 2026-07-01 go/no-go review re-walked this register against `ce7d719` and both EC2 runs. The current
> tally is done 98 / partial 25 / absent 2 / non-goal 6 / unmeasured 1 (honest band 95-98 done; three of
> the new done items -- 9.7 soak is 6 h not 24 h, 11.12 empirical is not exhaustive, 8.7 ACL is
> off-by-default -- are judgment calls, go/no-go §2.1; delta explained there). Several rows below are now
> stale relative to that reconciliation -- notably §2 items 2.6/2.7/2.9 (shown as deferred, but the
> multi-Raft server wiring has since landed on main and was exercised on metal), the §11.8 row text (says
> "deferred"; authoritative status is done at N=1), and the 1.12/10.2 "ADR-0030/0032 Proposed" notes (both
> are now Accepted). The individual status rows below have been refreshed to that reality with dated
> per-row reconciliation notes citing the go/no-go and the verifying artifact. The counts table immediately
> below is left as the frozen 2026-06-26 audit snapshot (a dated historical record, not re-tallied in
> place); the reconciled tally is the one in this note. The go/no-go review remains the authoritative
> source of truth.
>
> **Reconciled 2026-07-02.** A follow-on hardening pass closed several of the items the go/no-go carried as
> conditional or deferred. Against the per-row tables below this moves: 8.2 encryption at rest, absent to
> done (available behind a flag); 1.7 / 3.10 / 11.9 snapshot 4 MiB cap, partial to done (chunked
> InstallSnapshot lifts the ceiling); and adds three new done rows (4.13 caught-up-edge spurious
> gap-quarantine, 7.13 manual leadership transfer, 8.13 legacy SUBSCRIBE admission authorization). Net
> direction: the absent count drops from 2 to 1 (only 11.7 BATCH API remains genuinely absent). The frozen
> 2026-06-26 counts table below is not re-tallied; the individual rows carry dated per-row reconciliation
> notes. See §0.1 for the frozen-shortlist errata.
>
> **Honest remainder (what this pass did not close -- do not over-claim).** External KMS providers
> (AWS/Vault) plus ServiceLoader discovery (only `LocalDerivedKmsProvider` ships; §8.2); the multi-Raft
> leadership auto-balancer (only manual transfer is exposed; §7.13); multi-shard watches (§4.8 / §11.8); a
> conforming client driver (§6.6); audit-log encryption (audit stays HMAC-only; §8.2); groupId-in-AAD for
> cross-group at-rest integrity at N>1 (§8.2); a snapshot-drop / reassembly-cap-refusal metric (over-cap
> refusal is log-only today; §3.10 / D-7); disk-spilling snapshot reassembly (heap-bound today; §3.10).
> Plus the pre-existing deferred items (sharding sustained-N>1 leadership balancing, cross-region/WAN,
> BATCH API, full 24 h/72 h soak, literal sustained 10 k/s). (Several of these have since been built --
> see `docs/architecture/architecture.md` and the README for current status.)
...
**Optimistic to downgraded (the dangerous ones -- false done-marks that would have hidden a gap):**

| Item | Pre-fill | Audited | Why |
|---|---|---|---|
| 4.8 / 6.6 Watches | done | **done (N=1, server-side)** | The RFC §2 client-facing watch protocol is implemented server-side on the edge endpoint (`--edge-port`): the `0x02` wire (`WATCH_*` frames plus per-shard cursor vector), the multiplex/filter veneer, the whole-target authorization gate (reject-not-filter, fail-closed), per-watch target-filtered delivery plus catch-up snapshots, and bounded revocation under live ACL reload (W7-7). N=1 only (the multi-shard watch case shipped separately later, fail-closed until then). Caveats at the time: no shipped client driver yet; the watch ACL was conditional on segregating watch clients from the co-resident unauthenticated-beyond-mTLS legacy SUBSCRIBE path; single-scope keyspace at N=1 (see known-limitations §2). The legacy in-process `WatchService` is unrelated server-internal plumbing. |
| 10.12 Buggify | done | **partial** | `Buggify`/`BuggifyRuntime` built and self-tested but zero call sites anywhere; the deterministic sim injects faults via `SimulatedNetwork`, not Buggify. Named primitive is shelfware. |
| 5.11 BloomFilter | done | **partial** | Built and property-tested but zero `src/main` callers; the `DeltaApplier` half of the item is wired, the bloom half is dormant. |
| 9.7 24h soak | unmeasured/partial | **partial** | The soak launched (2026-06-14) and ran leak-clean (FD/threads/heap flat) but only ~3.45 h of 24 h before the Linux OOM-killer killed a node (box capacity, not a Configd leak). No full soak (24 h or 72 h) had completed at this point. |

**Pessimistic to upgraded (pre-fill too harsh -- memory under-credited shipped work):**

| Item | Pre-fill | Audited | Why |
|---|---|---|---|
| 5.2 Staleness measurement | partial | **done** | Leader commit-timestamp is load-bearing end-to-end (leader stamps -> wire `CommitNotification.commitTimestampMillis` -> edge ADR-0039 frontier -> `StalenessTracker`); the nanoTime idle-proxy was deleted. `consistency-contract.md` §2 line 74 and the staleness-invariant table row were stale at the time -- they still described the proxy. |
| 1.8 Pre-vote | partial | **done** | Dedicated discriminating test `RaftNodeTest.PreVoteTests.preVotePreventsTermInflationFromPartitionedNode`. (Caveat: pre-vote not in `ConsensusSpec.tla`.) |
| 6.4 Reconnect/failover | partial | **done** | `EdgeStreamClient` auto-reconnects round-robin carrying the resume cursor; `EdgeFailoverTest` proves it end-to-end (kill-mid-stream, cursor-monotonic, consistent refusal). |
| 8.5 Signing-key co-location | partial | **done** | Now fail-closed by default (`ConfigdServer.enforceSigningKeyNotColocated:1066` throws `SecurityException`; `D1FailClosedTest`; ADR-0043). Caveat below. |
| 8.10 Rate limiting | partial | **done** | Unconditionally wired (global 10k/s `ConfigdServer.java:646` plus per-principal `ConfigWriteService.java:283`) -- the only security control that is not off-by-default. |
| 9.3 Edge read perf | partial | **done** | Measured: `getHitWithCursor` p50=50 ns/p99=140 ns (JMH, gate enforced) plus 53,616 req/s @64 conn (m6i HTTP). |
| 9.12 Allocation profiling | partial | **done** | Measured: edge-read 14,999 to 1,704 B/req (8.80x) on production `NettyEdgeHttpServer`; read-path 0 B/op gated. |
| 6.10 SDK docs | absent | **partial** | `docs/wiki/Integration-Guide.md` has real edge-read/cursor/staleness examples -- but partial and stale (Gradle/Java-21 vs. actual Maven/Java-25). |
| 11.8 Watches scope decision | undecided | **done (scoped and built, N=1)** | The RFC §2 driver-protocol watch section (`docs/rfc/driver-protocol/02-watches.md`) scopes the client watch surface, and it is implemented server-side at N=1 (wire, veneer, authz, bounded revocation). Supersedes the prior "client-facing watches deferred" call for N=1; the multi-shard watch shipped later. |

### Documentation-staleness flags (found during audit -- docs trailing the code)

- **`docs/known-limitations.md` (2026-04-25) was half-stale.** Its "mutation-testing not measured", "jacoco
  not run", "allocation profiling not run" claims were obsolete -- mutation is measured and gated (70/70/65
  floors), allocation profiling is captured (8.80x), edge-read perf is gated. Still true at the time: JaCoCo
  line/branch coverage was not enforced anywhere, and no full soak had completed.
- **`docs/consistency-contract.md` §2 (line 74) and the staleness-invariant row were stale** -- they
  described the deleted nanoTime staleness proxy; the code uses leader commit-timestamps (see 5.2).
- *(These were flagged, not fixed, here -- this audit is the register only. A follow-up doc pass
  reconciled them.)*

---

## §0.1 Blocker shortlist and recommendation (the readiness review draws the actual ship line)

These are the auditor's judgments about which partial/absent/unmeasured items genuinely block an
unqualified production-ready claim, versus those that can ship as documented limitations. This is a
...
> (`docs/adr/adr-0030-quicksilver-shaped-topology.md:5`, `adr-0032-linearizability-harness.md:5`).
> #4 (DR drills never executed) -- executed on metal (372 ms failover, 0/1000 loss x3, RTO 4.2/5.9 s;
> `docs/archive/measurement/ec2-2026-06-30/02-dr-drills.md`). #1 (empirical validation deferred) --
> substantially discharged (6 h soak, DR drills, near-linear N x knee measured); residuals only (no
> 24 h/72 h soak, no literal 10 k/s, no WAN). #3 (encryption at rest) -- decided to defer at the time.
> #5 (live-cluster linearizability not in cloud CI) -- carried forward as a go/no-go condition. Net: no
> absolute, deployment-independent blocker survives, go-with-conditions. Everything in §0.1 below (the
> numbered blockers and the "documented limitations" list -- including its "sharding server wiring plus
> N x knee is a later measurement" bullet, now superseded: the wiring landed on main and the N x knee is
> measured, see §2.6-2.11) is the frozen 2026-06-26 auditor's snapshot; the reconciled per-row status is
> in §1-§11 and the go/no-go holds the decision.

> **Erratum 2026-07-01 (post-go/no-go) -- #7's "alert on the drop" mitigation does not exist.** The
...
> this drop. Detection was log-watch only (grep `snapshot too large for v1 wire`). Corrected live in
...
> **Erratum 2026-07-02 -- several frozen blockers are now closed in code.** This non-destructively
> supersedes the frozen shortlist rows below; the rows are left as-authored.
...
> (`IntegrityEnvelope.ALG_AES256_GCM=2`, `SegmentKeyManager`) plus a `KmsProvider` SPI plus
> `LocalDerivedKmsProvider` (HKDF from the signing key). The gap is closed for shipping (encryption can be
> turned on); external-KMS providers, audit-log encryption, and groupId-in-AAD for N>1 remained deferred at
> the time. The earlier "accept and defer" posture is superseded -- the system no longer has to ship
> without at-rest encryption. Caveat: enabling is a one-way door (once an `algId=2` record is written, no
> disable, no rollback). See §8.2 plus known-limitations section 1.
> **#7 (snapshot 4 MiB cap wedges a follower)** -- the total-state ceiling is lifted by chunked
> InstallSnapshot (`RaftNode` chunked transfer plus offset-echo; per-chunk cap 4 MiB; fail-closed
> reassembly cap `configd.raft.maxReassembledSnapshotBytes`, default 512 MiB). The "drop path" corrected
> above no longer exists for the dominant (over-4-MiB snapshot) case; the follower-wedge risk is replaced
> by a heap-bounded, fail-closed reassembly refusal that is log-only (a dedicated drop/refusal metric
> stayed a later observability follow-up). Hard deploy requirement: all nodes must be upgraded together (a
> pre-chunking follower installs chunk 0 as the whole snapshot, which is silent corruption). See §1.7 /
> §3.10 / §11.9 plus known-limitations "Snapshot transfer: chunked". Additionally, the frozen §0.1
> "documented limitations" notes on watches' SUBSCRIBE-path segregation and leadership operability are
> refined by the new rows §8.13 (SUBSCRIBE now authorized at admission when auth is on) and §7.13 (manual
> leadership transfer now exposed; the auto-balancer came later).
...
1. **Empirical validation is deferred to production observation** (11.12, 9.7, 9.8, 7.5). No completed
   ...
   This is a recorded explicit user choice -- so it blocks an unqualified claim but is shippable as an
   explicit burn-in / early-adopter posture. The single biggest ship-risk lever.
2. **ADR-0030 (single-Raft-group topology) and ADR-0032 (linearizability harness) were Proposed, not
   Accepted** (11.1, 10.2) at audit time. The structural decision the entire consistency contract rests on,
   and the ADR behind the linearizability proof, were formally unratified. Cheap to clear -- ratify or
...
3. **Encryption at rest is absent and is an open gap, not a decided non-goal** (8.2). Config values
   ...
   Blocks if the deployment stores secrets or has a compliance bar; otherwise needs an explicit decision to
   defer. (Edge at-rest exposure is bounded -- the edge store is in-memory.)
4. **DR drills have never been executed** (7.5). `ops/dr-drills/` holds only a README stating "no drills
   have been executed." Recommend running at least restore-from-snapshot and leader-loss drills once on
   the release commit before any operational-readiness claim.
5. **Live-cluster linearizability is not in cloud CI** (10.2). The trusted `anishathalye/porcupine`
   checker self-test (8/8) and sim-history are gated, but the live multi-process iptables-faulted matrix
   (the path that originally exposed the linearizability-bug race) is gated to self-hosted/nightly only.
   Recommend a green self-hosted faulted run captured on the release commit.
...
6. **Security is wired but off-by-default** (8.3, 8.6, 8.8, 8.9). Auth, TLS/mTLS, audit, replay are all
   ...
   secure-by-default / required-operator-config hardening and documentation item, not a code blocker.
7. **Snapshot 4 MiB cap** (1.7, 3.10, 11.9). A follower needing a >4 MiB `InstallSnapshot` wedges -- the
   over-cap frame is silently dropped to stderr (`RaftNode.java:2074-2078`). Ship as a documented
   limitation with the operational mitigation (keep snapshot <4 MiB; alert on the drop plus matchIndex
   lag); chunked install is a later fix. Blocks only if expected state >4 MiB.

### Ship as documented limitations (not blockers)

- **Sharding / multi-Raft production deployment** (2.6-2.9, 2.11) -- the default ships single-group by
  design (ADR-0030); the sharding logic is sim-verified, the server wiring and N x knee measurement came
  later.
- **Watches: server-side protocol done (N=1), no client driver yet** (4.8, 6.6) -- the RFC §2 watch wire
  protocol is implemented server-side; the remaining gap at the time was a conforming client driver plus
  the deployment security model (segregate watch clients from the legacy SUBSCRIBE path) plus the N>1
  case. Clients without the driver still pull via HTTP and the edge delta-stream.
- **Client-SDK maturity** (6.6-6.11) -- no write-SDK (writes are raw HTTP), Java-only, partial/stale docs.
- **Per-shard observability** (2.8, 7.10) -- irrelevant at N=1 (the default topology); becomes real only
  with sharding.
- **BATCH API** (11.7), **cross-region/WAN** (4.11, 9.11) -- planned/single-region at the time.
...
| 1.3 Commit + single-writer apply (owner-thread tripwire) | ✅ | single `applyCommitted()` loop `RaftNode.java:2186` + `assertOwnerThread()` `:493`; jcstress `RaftOwnerThreadGuardTest`; `apply_owner_thread` twin fires in `AssertionTwinFiringTest` | -- |
...
| 1.7 Snapshot install (chunked) | ✅ (was 🟡; reconciled 2026-07-02) | `handleInstallSnapshot():552` (SnapshotInstallSpec twins); `InstallSnapshotTest`, `SnapshotCrashRecoveryTest`; **chunked transfer** `ChunkedInstallSnapshotTest` | **Total-state 4 MiB ceiling LIFTED** -- a large snapshot streams as ordered chunks (per-chunk cap `MAX_SNAPSHOT_CHUNK_BYTES`=4 MiB) driven off the follower's echoed `nextExpectedOffset`; heap-bounded, fail-closed reassembly cap `configd.raft.maxReassembledSnapshotBytes` (default 512 MiB). **Deploy-together requirement** (a pre-chunking follower would install chunk 0 as the whole snapshot = silent corruption). Detail in §3.10 |
...
| 10.2 **Linearizability (Porcupine)** | ✅ † | Real `github.com/anishathalye/porcupine v1.2.0` (the etcd checker, not hand-rolled); gate-1 step b builds and runs `CheckerSelfTest` **8/8**; gate-2 checks sim-history linearizable (`SimHistoryCheck`); ADR-0032 | † the **live multi-process iptables-faulted matrix is gated to self-hosted/nightly only** (skipped in cloud CI) -- carried forward as go/no-go condition **C3**. **ADR-0032 discharged 2026-07-01: now Accepted** (ratified 2026-06-27; `docs/adr/adr-0032-linearizability-harness.md:5`) -- was *Proposed* at audit time. **C3 update:** the faulted-linz harness was fixed (each test node's signing key mounted outside its data dir via `--signing-key-file`, satisfying the real signing-key co-location guard rather than disabling it; `ClusterSigningKeyTest`) -- the matrix now runs green (8/8 linearizable, reproducibility gate iv byte-identical) on `9e1f191` (`docs/measurement/ec2-drive-to-green-2026-07-02/README.md`). Because later work changed consensus/storage after that capture, the definitive C3 was re-run on the release SHA. **Done 2026-07-02: green on release SHA `eb9b293`** -- 8/8 linearizable (3- and 5-node, faults active), reproducibility gate iv byte-identical (`docs/measurement/ec2-drive-to-green-2026-07-02/gate7-final/`). Condition C3 closed on the shipped bytes. |
...
> **Headline (as originally audited):** the sharding logic is fully sim-verified and CI-gated; the
> production server wiring was deliberately deferred at the time. The default deploys single-group (N=1),
> byte-identical to the unsharded design. N>1 was not deployable; the N x knee aggregate was unmeasured.
>
> **Reconciled 2026-07-01 (go/no-go §2.2, authoritative) -- this headline is now stale.** The server wiring
> has since landed on main and was exercised on metal: `ConfigdServer` wires `StaticShardMap` plus an
> `addGroup` loop plus `shardFor` routing (2.6/2.7/2.9 below), and the sharded aggregate N x knee is
> measured near-linear at 2.45x/3 machines (2.11/9.2). The default still defaults to N=1
> (byte-identical); N>1 plus the edge endpoint fail-closes unless explicitly opted in
> (`ConfigdServer.java:247-259`), and sustained N>1 additionally needed the leadership-balancing follow-up
> (go/no-go §3.2) -- since built, see `docs/architecture/architecture.md`.
...
| 2.10 **Rehoming / dynamic resharding** | ⛔ dormant | `MultiRaftDriver` rehoming "DORMANT in production"; **the re-verify-on-activation check does not fire** | activation requires re-verification |
...
| 3.7 durable_prefix_no_gap | ✅ | `applyCommitted` refuses to advance past a gap `RaftNode.java:2203`; recovery-side ctor check `:359`; `SnapshotCrashRecoveryTest.gapDetectionFires…` | -- |
...
| 4.4 FanOutBuffer evict-before-overwrite (ADR-0036) | ✅ | `FanOutBuffer` + `FanOutBufferRaceTest` | -- |
...
| 4.8 **Watches** | ✅ (N=1, server-side) | RFC §2 watch protocol implemented server-side on the edge endpoint: wire `0x02` + `WATCH_*` frames + cursor vector, multiplex/filter veneer, whole-target authz gate, target-filtered delivery + catch-up snapshots, **bounded revocation (W7-7)**; the unrelated legacy in-process `WatchService` stays server-internal | **Decision updated 2026-06-29: the N=1 client watch protocol is built server-side** (supersedes the 2026-06-27 "watches are deferred" decision for N=1). Caveats at the time: **no shipped client driver yet**; watch ACL conditional on SUBSCRIBE-path segregation (**reconciled 2026-07-02:** the legacy SUBSCRIBE feed is now authorized at admission on a whole-store READ cover when auth is ON -- §8.13 -- so the segregation caveat now applies only to the **auth-off / mTLS-only** posture); single-scope at N=1; **the multi-shard watch shipped later** (fail-closed until then). See known-limitations §2. |
...
| 5.12 **p99 staleness distribution** | ✅ (release-SHA re-run; was 🟡) | `EdgeStalenessDistributionLoadSimTest:69` asserts `p99<500ms`/`p9999<2s` over real frontier; `EdgeStalenessDistributionSimTest` (mechanism-only) | the real-bound test is **local-component-only, sim-level, not CI-pinned**; global/WAN p99 unmeasured. The on-metal distribution run was **blocked by the spurious edge gap-quarantine bug** (the mechanism itself validated -- a subscribed edge hovers 2-251 ms, bounded by the 250 ms heartbeat); that bug is now fixed (4.13). **Done 2026-07-02 on release SHA `eb9b293`, bound met by ~10-38x:** 4 edges/500 w/s/180 s p99 24 ms / p9999 117 ms; 1 edge/100 w/s/30 min p99 13 ms / p9999 212 ms / max 232 ms (bound p99 < 500 ms, p9999 < 2 s). A faithful deep-tail at high multi-edge density wants dedicated edge hardware (single-box co-location occasionally starves an edge JVM); the clean per-edge steady-state distribution is representative and meets the bound (`docs/measurement/ec2-drive-to-green-2026-07-02/gate7-final/`). |
...
| 7.13 **Leadership transfer (manual, ADMIN-gated)** | ✅ (new) | `POST /v1/admin/groups/{groupId}/transfer-leadership?target=<nodeId>` on `AdminApiHandler` (both HTTP adapters delegate); `DriverLeadershipAdmin` posts `RaftNode.transferLeadership` to the group owner-executor under a bounded deadline (wedged owner ⇒ 503, never blocks the HTTP thread); `AdminService` shapes outcomes (NotLeader ⇒ 503 + `X-Leader-Hint`, precondition ⇒ 409, Success ⇒ 200); Raft §3.10 transfer-abort so a bad target no longer wedges writes | **Manual only at this point -- the multi-Raft leadership auto-balancer came later** (go/no-go §3.2; the operability gap filed after the horizontal run; since built, see `docs/architecture/architecture.md`). Remedies post-failover leadership drift (which collapses the sharded aggregate toward the single-group plateau). Fail-closed: **refused when auth is OFF or ADMIN cannot be evaluated** (stricter than the config gate). Dormant/byte-identical when the seam is not wired (route ⇒ 404) |

## §8. Security

> **Two headline findings (as originally audited):** (1) config data was not encrypted at rest (integrity
> is not confidentiality), a registered open gap. (2) Security mechanisms are wired into the live path
> (the historical "interface-only" caveat is stale) but are off-by-default -- only rate-limiting is
> unconditionally on.
>
> **Reconciled 2026-07-02.** Finding (1) is addressed: at-rest encryption is available behind a flag (§8.2,
> absent to done) -- node-local AES-256-GCM plus a KMS-provider SPI. The default is still integrity-only
> (encryption off), so the "do not store secrets by default" posture stands. Finding (2) (off-by-default
> posture) was unchanged at this point and was hardened by separate secure-by-config work since. New at
> this reconciliation: §8.13 (legacy SUBSCRIBE authorized at admission when auth is on -- the "any valid
> mTLS cert pulls the whole store" gap is closed for the auth-on posture).
...
| 8.2 **Encryption at rest** | ✅ (available behind a flag; was ❌; reconciled 2026-07-02) | **node-local AES-256-GCM** at the ADR-0042 seam: `IntegrityEnvelope` `ALG_AES256_GCM=2` (GCM tag replaces the HMAC; CRC32C layer stays), `SegmentKeyManager`, `AtRestKeys`; enabled by `-Dconfigd.raft.encryption.enabled` / `CONFIGD_ENCRYPTION_AT_REST` (`ConfigdServer.java:1202`); KMS-provider SPI `KmsProvider` + default `LocalDerivedKmsProvider` (HKDF from the signing key, domain-separated); `IntegrityEnvelopeEncryptionTest`, `RaftLogEncryptionTest`, `SegmentKeyManagerTest`, `LocalKmsEncryptionIntegrationTest`, `EncryptionAtRestWiringTest`; gate-7. **The gap is closed for shipping** | **Off by default** (default is integrity-only; "do not store secrets" still applies with encryption off). **One-way door:** once any `algId=2` record is written, encryption cannot be disabled and the binary cannot be rolled back (recovery fails closed). Encrypts WAL/snapshot/durable-Raft-state; **audit log stays HMAC-only at this point**. No wire change, no cluster key distribution. **Deferred at this point:** external KMS providers (AWS/Vault) + ServiceLoader discovery, audit-log encryption, groupId-in-AAD for cross-group at-rest integrity at N>1 (several since built, see `docs/architecture/architecture.md`). `secure/` remains a read-*freshness* class, not confidentiality. **Write-path overhead measured 2026-07-02 (release SHA `eb9b293`, single node, 256 B):** throughput knee 1210 -> 1180 w/s (-2.5%), commit p50 7.65 -> 7.77 ms (+1.5%), p99 14.6 -> 36.4 ms (+150%) - low on throughput/median, tail-weighted (per-record AES-GCM + allocation); local-encrypt cost only, no replication fsync in the path (`docs/measurement/ec2-drive-to-green-2026-07-02/gate7-final/`). |
...
## §11. Cross-cutting scope decisions
...
| 11.5 Durability Level 0/1, no early-ack | ⛔ | `handoff.md:123`; grep finds **no early-ack path** in consensus/replication/server | -- |
...
| 11.8 Watches scope decision | ✅ (N=1; was deferred; reconciled 2026-07-01) | **Superseded 2026-06-29:** the RFC §2 client watch protocol is implemented server-side at N=1 -- wire `0x02` + `WATCH_*` frames + per-shard cursor vector, whole-target authz gate, bounded revocation (see §4.8/6.6). `docs/rfc/driver-protocol/02-watches.md`. Matches the §0 watch note + go/no-go §2.2 | **The multi-shard watch shipped later** (fail-closed until then); no shipped client driver yet at this point. The prior "watches are deferred" call held only for the window before this arc |
| 11.9 Snapshot transfer chunked (4 MiB ceiling lifted) | ✅ (was 🟡; reconciled 2026-07-02) | chunked `InstallSnapshot`; known-limitations "Snapshot transfer: chunked"; `ChunkedInstallSnapshotTest` | followers can now bootstrap from an arbitrarily large snapshot (streamed as chunks), bounded by follower heap + a fail-closed 512 MiB reassembly cap; deploy-together requirement. See §1.7 / §3.10 |
| 11.10 Wire epoch field deferred | 🟡 | in-memory `ShardMap.epoch()` returns 0; wire field deferred | **Open operator decision** (reserve the field now vs. accept a later wire break) |
...
| D-2 | **Resolved 2026-07-02 -- encryption at rest built and available behind a flag**; supersedes the 2026-06-27 decision to defer it | 8.2 ✅ (was ❌); the gap is closed for shipping | ✅ done -- encryption is available (node-local AES-256-GCM, `-Dconfigd.raft.encryption.enabled`, off by default) plus a KMS-provider SPI. One-way door once enabled. Default stays integrity-only so "do not store secrets (with encryption off)" still applies; external KMS / audit-log encryption / groupId-in-AAD (N>1) were deferred at this point |
...
| D-4 | **Empirical-validation / burn-in posture** | 9.7 soak incomplete; 7.5 drills never run; 11.12 | accept the "first-30-days = burn-in" contract, or gate shipping on a completed soak plus DR drills |
| D-5 | **Wire epoch reservation** | in-memory epoch present; wire field deferred | reserve now (one `WIRE_VERSION` bump) vs. accept a later wire break |
| D-6 | **Sub-second region failover (ADR-0031 GA blocker)** | Accepted ADR declares it a GA blocker; not built | confirm the shipped design goes out without it (single-DC) and it's a tracked GA gate |
| D-7 | **Resolved 2026-07-02 -- chunked InstallSnapshot shipped**; 4 MiB total-state ceiling lifted | 1.7/3.10/11.9 ✅; heap-bounded fail-closed reassembly cap (default 512 MiB) | ✅ done via chunked install (not "accept-as-limitation") -- the over-cap **drop path is removed** for the snapshot case. Residual: **deploy-together requirement** (silent corruption if mixed with a pre-chunking node) plus the reassembly-cap refusal is **log-only** (a dedicated drop/refusal metric stayed a later observability follow-up) |
| D-8 | **Dead code: BloomFilter / Buggify / RolloutController** | 5.11/10.12/4.10 dormant, zero callers | wire or delete before shipping (avoid shipping shelfware that reads as "done") |
