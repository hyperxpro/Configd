# Hierarchical Path / Namespace Model — Design & first RFC section

**Status: design complete (2026-06-28). Docs-only. No production code** (a compile-checked type sketch
under [`sketch/`](sketch/) is a design artifact, not wiring). This is a *design + recommendation + the
first RFC section*, not a build.

The operator's decisions are settled and this designs **to** them: Configd's v1 config model is
**hierarchical and path-based** (ZooKeeper / Apache Curator / HashiCorp Vault-secret-path style), **not**
flat keys; read/write ACLs are live and a **WATCH** verb is needed; and the **driver-protocol RFC is
written alongside as decisions land** — this session produces its first section. The built model today is
a **flat, hash-sharded keyspace** (`String key` + `ConfigScope`) with **per-prefix ACLs**; the
hierarchical namespace model (ADR-0017) was designed but **never built — paper**. This session designs the
real v1 path model + the ACL extension and drafts the RFC section the wiring will conform to.
...
4. **[`../../rfc/driver-protocol/01-paths-and-access.md`](../../rfc/driver-protocol/01-paths-and-access.md)**
   — the **first RFC section**, normative (MUST/SHOULD/MAY), rigorous enough that drivers in any language
...
slash-delimited hierarchy that is the **logical** structure for ACLs, watches, listing, and management —
but **`shardFor` hashes the whole path**, so a subtree **scatters across all shards** and every subtree
...
posture (scatter-gather scan v1, ordered index v2) · no recursive delete in v1. Full list:
