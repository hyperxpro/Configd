...
Design work from 2026-06-28, docs-only, no production code. Companion to [`path-model.md`](path-model.md)
(the Configd design), [`access-control.md`](access-control.md) (the ACL extension), and the first
driver-protocol RFC section
...
(its ACLs are per-node and non-recursive, so ZK cannot even express "authorize this whole subtree" cleanly) -- a gap our inherited-down model closes.
...
bounded** (a recursive `list` is a paginated scatter-gather; there is no implicit recursive *delete* -- a
subtree spans all shards and a cross-shard atomic delete does not exist, ADR-multiraft-cross-shard).
...
split is folded into the coarse `WRITE` for now (noted as a later refinement), and `WATCH` is added
...
unit is still `(ConfigScope, String key)` (§0). ADR-0017 is paper, and this design **supersedes it where it
conflicts with the hash-routing invariant or the built reality**. The reconciliation, item by item:
...
| Per-namespace **ACL policies**, **rate limits**, **quotas**, **lifecycle** (create/configure/drain/delete) stored under `/_system/namespaces/` | **Keep as an optional logical overlay (a later item).** A "namespace registry" MAY attach policy/quota metadata to a top-segment prefix, stored under a reserved subtree (e.g., `/_acl/`, `/_system/`), replicated as ordinary config. It remains a **logical overlay, never a routing input**. | Compatible with the invariant as long as it does not route. Deferred: multi-tenancy is delivered by subtree ACLs + per-principal limits; the typed registry/quota/lifecycle is additive later. |
...
| Namespace isolation, per-tenant limits, lifecycle/quota registry | ADR-0017 | **Keep the goals** via subtree ACLs + per-principal limits + a later logical registry |
