...
Design research from 2026-06-28, no production crypto, no provider modules built at the time. Reads on
from the encryption-at-rest research
...
fail-closed requirements that any implementer MUST honor, the zero-dependency default provider, one cloud
...
- It **MUST NOT** expose per-record `encrypt`/`decrypt`. That omission is load-bearing: a per-record method
  is exactly how an implementer would (accidentally or lazily) put a KMS round-trip on the write/replay path -- the correlated-dependency liveness trap §3 dissects. The data-plane cipher is the **core's** job, run on
...
sketch, not compiled against the SDK at the time this was written -- confirm against `prior-art.md` §3 /
the SDK Javadoc when built.*
...
(`CryptographyClient.wrapKey/unwrapKey`), `configd-kms-gcp` (`KeyManagementServiceClient.encrypt/decrypt` -- no `GenerateDataKey`, generate the DEK locally then wrap), `configd-kms-vault` (`transit/encrypt` ·
...
- **No custom crypto** -- providers wrap established KMS APIs (AWS/Azure/GCP/Vault) and standard JCA types;
  the default uses HKDF. No primitive is rolled here.
...
All four are **designed-in now** so that when the encryption work (Option D) is built, the providers slot
in with **no core change** -- interface-first, as intended.
