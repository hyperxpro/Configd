...
**Research session, 2026-06-27/28. Docs-only. No code, no implementation.**

Purpose: extract the *mechanisms* (not summaries) of change-subscription in three mature systems —
etcd (the gold standard, studied hardest), Consul, and ZooKeeper — so the Configd watch design
...
The next document maps this target onto Configd's sharded, failover-prone, edge-fronted architecture —
where the single global revision is replaced by a **per-shard cursor vector**, and the "serve from a
