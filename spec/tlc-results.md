...
All three specs run in CI (`.github/workflows/ci.yml`). Results below are from a live
run on Corretto 25 with `tla2tools.jar` v1.8.0.
...
- **ConsensusSpec: MaxTerm 3 -> 2.** The `acked` variable (capped to one ack position),
  `ClientAck`, and `AckImpliesCommitted` multiply the reachable space against the
  term/reconfig product. At MaxTerm=3/MaxLogLen=3 the full run does not exhaust in 40 min
  (16.6M states / 4.19M distinct / depth 19, 1.2M queued, 0 violations in the explored
  prefix). At MaxTerm=2 it exhausts (2.285M distinct, No error, 28m29s). Election +
  re-election, joint reconfiguration, client writes, edge propagation, and the
  commit-confirmed ack cycle all stay reachable. The ack-before-commit defect is proven at
  full term dynamics separately by `ConsensusSpec-ackonappend.cfg`.
- **SnapshotInstallSpec: MaxIndex 4 -> 2.** The durability model (4 new per-node variables
  + always-enabled `CrashRestart`) is a fundamental state-space change; see the per-spec
  note below.
- **ReadIndexSpec** keeps its original bounds -- the `~HasServeableRead` liveness gate
  slightly reduces the safety space (2.036M vs 2.276M distinct without it), so no change
  was needed.

Two counterexample configs prove the corresponding invariants are non-vacuous -- see the
ConsensusSpec and SnapshotInstallSpec sections below.

## ConsensusSpec
- **Config:** `ConsensusSpec.cfg` -- Nodes={n1,n2,n3}, MaxTerm=2 (reduced from 3 -- see
  Summary), MaxLogLen=3, Values={v1,v2}, `ACK_ON_APPEND=FALSE`
- **Full result:** 9,735,403 states / 2,285,001 distinct / No error / 28m29s.
...
- **AckImpliesCommitted non-vacuity (test-the-tester):** `spec/ConsensusSpec-ackonappend.cfg`
  ...
  local append (pre-commit) is truncated on a leader change and lost. Capture:
...
- **Full result:** 10,743,169 states / 2,036,131 distinct / No error / 4m25s.
- **Liveness:** `LiveSpec == Init /\ [][Next]_vars /\ FairNext` (weak fairness on
  ...
  **Wrong-fairness vacuity proven:** the same property under the UNFAIR base `Spec`
  ...
  liveness property model-checked in this repo.
- **Non-vacuity:** `ReadFreshness` and `NoStaleLeaderServe` are non-vacuous checks, not
  tautologies that could never fail: `ReadFreshness` asserts every served read's
  `readIdx <= appliedIndex[server]` (a read never reports ahead of the applied state);
  `NoStaleLeaderServe` asserts every served read's `term <= currentTerm[server]`. Both are
  proven non-vacuous by a seeded buggy-action counterexample and still model-check green.

## SnapshotInstallSpec
- **Config:** `SnapshotInstallSpec.cfg` -- Nodes={n1,n2,n3}, MaxTerm=2, MaxIndex=2 (reduced
  from MaxTerm=3/MaxIndex=4 for the durability model -- see the cfg header rationale),
  `PERSIST_BEFORE_TRUNCATE=TRUE`
- **Invariants (live):** TypeOK, SnapshotBoundedByCommitted, SnapshotMatching, NoCommitRevert,
  InflightTermMonotonic, DurablePrefix, RecoveredCoversCommitted -- **all PASS**.
...
- **Full result (MaxIndex=2):** 18,112,899 states / 1,797,096 distinct / No error / 13m26s.
  (At MaxIndex=4 the durability model is intractable; MaxIndex=2 exhausts and still
...
- **DurablePrefix non-vacuity (test-the-tester):** `spec/SnapshotInstallSpec-truncatebeforepersist.cfg`
  ...
  neither the durable snapshot nor the durable WAL. Capture:
  `docs/session-2/captures/spec-truncate-before-persist-counterexample.txt`.
- **Non-vacuity:** `NoCommitRevert` is a non-vacuous check, not a `P \/ ~P` tautology: it
  asserts that an in-flight InstallSnapshot which would install (`lastIncludedIndex >
  snapshot[to].index`) must carry `lastIncludedTerm >= snapshot[to].term` (a higher-index
  snapshot never reverts the term). Proven non-vacuous by a seeded-bug counterexample and
  still green.

---

## Bugs Found and Fixed During Model Checking (ConsensusSpec)
