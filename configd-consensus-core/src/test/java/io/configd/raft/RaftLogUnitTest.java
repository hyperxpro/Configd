...
class RaftLogUnitTest {
    ...
    class TermAt {
        ...
        void zeroSentinelGuardMattersWhenSnapshotIsAhead() {
            // With snapshotIndex > 0, removing the `index == 0` guard would let
            // termAt(0) fall through to `index < snapshotIndex` and return -1 instead
    ...
    class EntriesBatch {
        ...
        void stopsBeforeExceedingMaxBytesAfterFirst() {
            ...
            // would push the total to 4 > 3, so it stops at 1.
            RaftLog log = logWith(1, 1, 1);
    ...
    class CommitAndApply {
        ...
        void setCommitIndexAtEqualValueReClampsAfterTruncateShrinksLog() {
            // setCommitIndex uses a strict `>` guard, not `>=`: at commitIndex ==
            // lastIndex the guard skips and leaves commitIndex untouched. This matters
    ...
    class Compact {
        ...
        void compactExactlyAtLastIndexKeepsNoEntriesViaPartialPath() {
            ...
            // false), removing the whole prefix [1..3] inclusive. The guard must stay
    ...
    class Recovery {
        ...
        void tornSnapshotBlobIsTreatedAsAbsent() {
            ...
            // bounds checks (dataLen vs remaining) must reject it, leaving
        ...
        void blobShorterThanHeaderIsRejectedAtBoundary() {
            // Header is 8+8+4 = 20 bytes: 19 bytes must be rejected as null; a full
            // 20-byte header with dataLen 0 and a 4-byte cfgLen(-1) is a valid empty
            // snapshot.
        ...
        void legacyWalWithoutMetaInfersSnapshotIndexFromFirstEntry() {
            ...
            // prior compaction at index 2. The legacy fallback must infer
