 ...
 * as {@link ErrorCode#AUTH_FAIL} (4). The HTTP {@code 401} class maps here too (Gate 4).
 *
 * <p><b>§07 reaction:</b> <b>(re)authenticate</b> — fix the certificate or present a valid token/basic
 * credential. A rejected pre-auth {@code AUTH} closes the connection, so a retry costs a <b>fresh
 * connection</b> — a driver <b>MUST NOT hot-loop</b> {@code AUTH} frames on one connection (§03 AU4-4).
