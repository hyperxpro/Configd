 ...
 * <p>
 * Both the JDK {@link TcpRaftTransport} and the Netty {@code NettyRaftTransport} (M4 / DR-N16)
 * implement this interface, so the production cutover flips a <b>single construction line</b>
 * ({@code new TcpRaftTransport(...)} → {@code new NettyRaftTransport(...)}) — the DR-N14
 * {@code FanOutEndpoint} fast-revert pattern applied to the consensus surface. The
...
public interface RaftTransportEndpoint extends RaftTransport, AutoCloseable {
     ...
     * this is the RR-002/RR-054 metric seam.
     *
     ...
     * (F-S7-FUZZ-1 slowloris / FD-exhaustion guard). Monotonic.
     *
