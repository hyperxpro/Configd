#!/usr/bin/env bash
# =============================================================================
# gate-5.sh — Configd Session-5 cumulative machine-verifiable gate (PERFORMANCE)
# -----------------------------------------------------------------------------
# Authored by the Session-5 performance-lead. Cumulative with gates 1+2+3+4: a
# green gate-5 REQUIRES a green gate-4 (which chains 3→2→1). In CI gate-4 runs as
# its own job, so the gate-5 job sets GATE5_SKIP_GATE4=1 (cumulative coverage via
# the job dependency, not a redundant re-run) — reported LOUDLY. Exits non-zero on
# ANY failure; NO silent placeholders (RR-012/RR-085 lesson); every step asserts a
# real result and FAILS if its summary line is absent (non-vacuity).
#
# WHAT A GREEN GATE-5 PROVES (charter §11 — locks in the Session-5 wins so
# Sessions 6-8 cannot silently erode them):
...
#                   getMiss + getIntoHit < 1 B/op) — the §0.1 read law, mechanical.
#   (c) read-tail   read-path p99/p999 latency REGRESSION thresholds (JMH
#                   SampleTime / HdrHistogram). Measured S5: getHitWithCursor@100k
...
#   (e) backpressure  the §11 as-built write bound (Workstream D / RR-110):
#                   RaftConfig default maxPendingProposals == 1024 (the real
#                   bounded-proposal-queue threshold). Locks the documented bound.
...
# manifest M-9), the 24h soak, the live Compose staleness run, the GC bake-off.
# Those are docs/session-5 captures + the perf nightly lane, not the fast gate.
#
...
# --- (b) read-path zero allocation (reuse the CT-34 mechanical gc gate) --------
...
# --- (e) §11 backpressure bound (Workstream D / RR-110): maxPendingProposals=1024
