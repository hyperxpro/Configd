...
# (a) ShardMap + StaticShardMap (hash-within-scope) + the DISCLAIM guard
...
assert_class_green "$C1" "StaticShardMapTest"        # the hash-within-scope ownership model
assert_class_green "$C1" "CrossShardWriteGuardTest"  # cross-shard multi-key BATCH rejected (DISCLAIM)
echo "gate-phase1 c1: OK"

# (b) the multi-shard simulator + the six invariants, proven NON-VACUOUS
...
# GATE_PHASE1_SWEEP_COUNT sizes the full-surface sweep (fast PR = 200; nightly = 10000).
...
# the shard-map invariants + the operator-flagged wire-epoch deferral are recorded
...
# (d) Server-wiring foundation: Seam A (config N) + Seam B (inbound demux)
...
assert_class_green "$WIRING" "ShardCountConfigTest"   # range + N>1 BOOTS (Seam G4) + fixed-at-deploy reshard reject
assert_class_green "$WIRING" "RaftInboundDemuxTest"   # gid=k -> group k (not 0); hostile gid dropped
...
# (g) Server-wiring Seam E: per-shard observability (not limited to group 0)
...
# (h) Server-wiring Seam F: the WIRE_VERSION 0x01->0x02 bump (epoch reservation + coalesced frame)
...
assert_class_green "$WIRINGF" "FrameCodecEpochReservationTest"            # epoch MBZ + N=1 byte-identity
...
assert_class_green "$WIRINGF" "CoalescedHeartbeatCodecTest"               # coalesced codec round-trip + bounds
assert_class_green "$WIRINGF" "RaftTransportAdapterCoalescedInboundTest"  # inbound per-group demux
assert_class_green "$WIRINGF" "HeartbeatDrainFramingTest"                 # send drain dormant at N=1
assert_class_green "$WIRINGF" "RedTeamCoalescedWirePoCTest"               # adversarial parser battery (red-team)
...
# The cumulative proof that N>1 is correct END-TO-END — the gate on lifting the boot
...
# Ordered AFTER wiring-g (the integrated sweep) so the structure encodes the gate: the guard
